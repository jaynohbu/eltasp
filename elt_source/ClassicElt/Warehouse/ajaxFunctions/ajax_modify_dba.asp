...
sub register_colo(orgNum)
	...
	SQL = "INSERT INTO colo ( coloder_elt_acct,coloder_name,colodee_elt_acct,colodee_name, coloder_org_num,tran_date ) " &_
          " VALUES ( "& v_agent_elt_acct & ",'" & v_dba_name & "'" & "," & elt_account_number &_
          ",'"& Request.Cookies("CurrentUserInfo")("company_name") & "'" &_
          ","& orgNum & ",'"& now & "' )"
	eltConn.Execute(SQL)
end sub
%>
...
'////////////////////////////////////////////////////////////////////////////////
'	v_defaultBrokerName
'	if(v_b_country_code <> "") then register_code( v_b_country_code, 1)
'	if(v_b_country_codeName <> "") then register_code( v_b_country_codeName, 1)
'	if(v_owner_mail_country <> "") then register_code( v_owner_mail_country, 1)
end sub
%>
<%
sub register_code( clsVal, cNum )
	...
	DIM rs,SQL
	clsVal = UCASE(clsVal)
	SQL = "select code from all_code where elt_account_number =" & elt_account_number & " and type =" & cNum & " and UPPER(code)='" & clsVal & "'"
	Set rs = eltConn.Execute(SQL)
	If rs.EOF or rs.BOF then
		SQL = "insert into all_code (elt_account_number,type,code) values (" & elt_account_number & "," & cNum & ",'" & clsVal & "')"
		eltConn.Execute(SQL)
	end if
	Set rs=Nothing 
...
<%
	...
	loop
	put_recent_work "/IFF_MAIN/ASP/MASTER_DATA/client_profile.asp?n="&v_o_num,_
					v_o_num,_
...
<%
	...
	put_recent_work "/IFF_MAIN/ASP/MASTER_DATA/client_profile.asp?n="&v_org_account_number,_
...
<%
sub get_db_field
			v_dba_name				= Request("txt_dba_name")
			v_class_code			= Request("txt_class_code")			
