...
SQL = "UPDATE all_accounts_journal set chk_void = '"&how&"' where elt_account_number="&elt_account_number&" and tran_seq_num in " & Tran_seq_num
eltConn.Execute(SQL)
call get_chk_result
end sub
%>
...
if isnull(Tran_seq_num) then exit sub
Tran_seq_num = OTL(Tran_seq_num)
SQL = "UPDATE all_accounts_journal set chk_complete = '"&how&"' where elt_account_number="&elt_account_number&" and tran_seq_num in " & Tran_seq_num
eltConn.Execute(SQL)
call get_chk_result
end sub
%>
...
'response.write Bank & chr(13)
'response.write startDate & chr(13)
'response.write endDate & chr(13)
...
subSQL = "   FROM  all_accounts_journal a, gl b" &_
		...
		if Bank <> "0" and  Bank <> "" then
			subSQL = subSQL & "	  AND a.gl_account_number = '" & Bank & "'"
		end if
...
SQL =	"SELECT a.elt_account_number, tran_seq_num, tran_type as Type,convert(char(10),tran_date,101) as Date,"&_
