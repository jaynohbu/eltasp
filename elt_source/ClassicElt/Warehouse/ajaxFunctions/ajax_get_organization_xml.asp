...
<% 
    ...
    Function get_organization_list(org_type)
        ...
        Select case org_type
            ...
            case "Pickup"
                SQL = SQL & " AND ((is_consignee='Y' AND ISNULL(carrier_code,'')<>'') OR is_shipper='Y') "
            case "CarrierWarehouse"
                SQL = SQL & " AND ((is_carrier='Y' AND ISNULL(carrier_code,'')<>'') OR z_is_warehousing='Y') "
            case "Customer"
                 SQL = SQL & " AND (is_shipper='Y' OR is_consignee='Y' OR is_agent='Y' OR z_is_govt='Y' OR z_is_broker='Y') "

                
        End Select
        
        If qStr <> "" Then
            SQL = SQL & " AND UPPER(dba_name) LIKE '" & UCase(qStr) & "%'"
        End If
