...
Function GetListByHAWB(hawbNum,eltNum,tableName)
    ...
    If hawbNum = "" Then
        sqlTxt = "SELECT HAWB_NUM,MAWB_NUM FROM " & tableName & " WHERE MAWB_NUM='" _
            & mawbNum & "' AND elt_account_number='" & eltNum  & "' AND iType='" & AE & "'"
    Else
        sqlTxt = "SELECT HAWB_NUM,MAWB_NUM FROM " & tableName & " WHERE HAWB_NUM='" _
            & hawbNum & "' AND elt_account_number='" & eltNum  & "' AND iType='" & AE & "'"
    End If
