...
'/////////////////////////////////////////////////// by ig NOV-27
    ...
    if Not COLODee="" then
	    ...
	    if rsHAWB.EOF then
		    ...
		    rsHAWB.close
		    COPY="CONSIGNEE"
		    SQL= "select * from mawb_master where elt_account_number = " & COLODee & " and MAWB_NUM='" & vMAWB & "'"
	    end if
    else
...
'///////////////////////////////////////////////
    ...
    rsHAWB.Close
    if Not COLODee="" then
	    SQL= "select * from mawb_weight_charge where elt_account_number = " & COLODee & " and MAWB_NUM='" & vMAWB & "' order by tran_no"
    else
    ...
    'if (Copy="SHIPPER" and (vShowPrepaidOtherChargeShipper="Y" or vShowCollectOtherChargeShipper="Y")) or (vShowPrepaidOtherChargeConsignee="Y" or vShowCollectOtherChargeConsignee="Y") then
	    if Not COLODee="" then
		    SQL= "select coll_prepaid,Amt_MAWB,Charge_Desc from mawb_other_charge where elt_account_number = " & COLODee & " and MAWB_NUM='" & vMAWB & "' order by tran_no"
	    else
...
%>
<!--  #INCLUDE VIRTUAL="/IFF_MAIN/ASP/include/manifest_pdf_air.inc" -->
<%
end Sub
...
rs.close
SQL= "select * from ocean_booking_number where elt_account_number = " & elt_account_number & " and booking_num='" & vMBOL & "'"
rs.Open SQL, eltConn, , , adCmdText
if Not rs.EOF then
...
SQL= "select * from mbol_master where elt_account_number = " & elt_account_number & " and booking_num='" & vMBOL & "'"
rs.Open SQL, eltConn, , , adCmdText
if Not rs.EOF then
...
if not vMBOL="" then
	if vAgent=0 or vAgent=MasterAgentNo then
		SQL= "select hbol_num,agent_name,pieces,gross_weight,measurement,Shipper_Info,Consignee_Info,manifest_desc from hbol_master where elt_account_number = " & elt_account_number & " and booking_num = '" & vMBOL & "' order by hbol_num"
	else
		SQL= "select hbol_num,agent_name,pieces,gross_weight,measurement,Shipper_Info,Consignee_Info,manifest_desc from hbol_master where elt_account_number = " & elt_account_number & " and booking_num = '" & vMBOL & "' and agent_no=" & vAgent & " order by hbol_num"
	end If
...
If rs.EOF then
	rs.close
	if vAgent=0 or vAgent=MasterAgentNo then
		SQL= "select mbol_num,'' as hbol_num, agent_name,pieces,gross_weight,measurement,Shipper_Info,Consignee_Info,manifest_desc from mbol_master where elt_account_number = " & elt_account_number & " and booking_num = '" & vMBOL & "' order by hbol_num"
	else
		SQL= "select mbol_num,'' as hbol_num,agent_name,pieces,gross_weight,measurement,Shipper_Info,Consignee_Info,manifest_desc from mhbol_master where elt_account_number = " & elt_account_number & " and booking_num = '" & vMBOL & "' and agent_no=" & vAgent & " order by hbol_num"
	end If
	rs.Open SQL, eltConn, , , adCmdText
End If
...
Sub InsertHBOLIntoPDF(vHBOL)
    ...
    '///////////////////////////////////////////////////////////////////////////////////////////////////////////
		    ...
		    'vDesc4="These commodoties, thecnology, or software were exported from the United States in accordance with the Export Administration Regulations. Diversion contrary to U.S. law prohibited"
		    vDesc4=rsHBOL("desc4")
		    vDesc5=rsHBOL("desc5")
    ...
    'get country stmt
    UserCountry=Session("user_country")
    SQL= "select * from form_stmt where form_name='bol' and country='" & UserCountry & "' order by stmt_name"
    rsHBOL.Open SQL, eltConn, , , adCmdText
    Do While Not rsHBOL.EOF
