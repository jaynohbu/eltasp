 /*
  * Infragistics WebGrid CSOM Script: ig_WebGrid_kb.js
	...
	switch(key)
	{
		...
		case 35: 
			...
			else
			{
				...
				if(nextRow.getHidden())
					nextRow=nextRow.getPrevRow(true);
				if(nextRow==row)
					nextRow=null;
			}
			if(nextCell || nextRow)
		...
		case 36: 
			...
			else
			{
				...
				if(nextRow.getHidden())
					nextRow=nextRow.getNextRow(true);
				if(nextRow==row)
					nextRow=null;
			}
			if(nextCell || nextRow)
		...
		case 37: 
			if(cell)
			{
				...
				do
				{
					...
					if(!nextCell)
					{
						if(cell.Row.getPrevRow(true))
						{
							nextCell=cell.Row.getPrevRow(true).getCell(cell.Row.getPrevRow(true).cells.length-1);
							if(!nextCell.Column.getVisible())
								nextCell=nextCell.getPrevCell();
						}
					}
		...
		case 39: 
			if(cell)
			{
				...
				{	
					...
					if(!nextCell)			
						if(cell.Row.getNextRow(true))
						{
							nextCell=cell.Row.getNextRow(true).getCell(0);
							if(!nextCell.Column.getVisible())
								nextCell=nextCell.getNextCell();
						}
					possibleNextCell=nextCell;	
			...
			break;
		case 38: 
			if(cell && cell.Row.getPrevRow(true))
			{
				var nr=cell.Row.getPrevRow(true);				
				while(!nextCell && nr)
				{
					nextCell=nr.getCellByColumn(cell.Column);
					nr=nr.getPrevRow(true);
					
				}
			}
			else if(row)
				nextRow=row.getPrevRow(true);
			if(nextCell || nextRow)
				processed=true;
			...
			break;
		case 40: 
			if(cell && cell.Row.getNextRow(true))
			{
				var nr=cell.Row.getNextRow(true);
				while(!nextCell && nr)
				{
					nextCell=nr.getCellByColumn(cell.Column);
					nr=nr.getNextRow(true);
				}
			}
			else if(row)
				nextRow=row.getNextRow(true);
		...
		case 46: 
			...
			break;
		default:
			if(key>=48 && key<=57 || key>=54 && key<=90 || key>=96 && key<=111 || key>=186 && key<=192 || key>=219 && key<=222 || key==113 || key==107 || key==109)
			{
