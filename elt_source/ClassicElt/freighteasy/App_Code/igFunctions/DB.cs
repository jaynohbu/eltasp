        ...
        public bool AUTH_CHECK(string strEltAcct,string strUserId, string strConn, string strURL, string strQuery)
        {
            ...
            if (login_name != "admin" && user_right != "9")
            {
                ...
                if (strEltAcct == null)
                {
                    ...
                    script += "alert('Your session was expired or disconnected!');";
                    script += "self.close();";
                ...
                string Query = "SELECT a.Authority_Id,b.Name, b.PhysicalPage FROM SE_User_Authority a, SE_Pages b WHERE a.Page_Id=b.Page_Id AND a.elt_account_number=" +
                                strEltAcct + " AND UserID=" + strUserId + " AND b.PhysicalPage = '" + strcurPageName + "'";
                ...
                {
                    mReader.Close();
                    mCmd.CommandText = "SELECT PhysicalPage FROM SE_Pages WHERE PhysicalPage = '" + strcurPageName + "'";
                    mReader = mCmd.ExecuteReader();
                    if (!mReader.Read())
                    {
                        rValue = false;
...
//////////////////////////////////////////////// view_login check
            ...
            if (mReader.Read())
            {
                if (mReader["user_name"].ToString() == "invalid")
                {
                    mReader.Close();

                    Query_login = "DELETE view_login where elt_account_number=" + strEltAcct + " AND ip='" + System.Web.HttpContext.Current.Request.Cookies["CurrentUserInfo"]["IP"].ToString() + "'" + " AND server_name='" + System.Web.HttpContext.Current.Request.Cookies["CurrentUserInfo"]["Server_Name"].ToString() + "'";
                    mCmd.CommandText = Query_login;
                    ...
                    if (mReader.Read())
                    {
                        another_user = mReader["ip"].ToString();
                    }
                    string script = "<script language='javascript'>";
                    script += "alert('Your session was disconnected by another user! ('+'" + another_user + "' +')' );";
                    script += "self.close();";
                    script += "top.location.replace('/freighteasy/index.aspx');";
                    script += "</script>";
                    System.Web.HttpContext.Current.Response.Write(script);
                ...
                else
                {
                    ...
                    {
                        mReader.Close();
                        Query_login = "Update view_login Set alive = 1, u_time='" + System.DateTime.Now.ToString() + "',requested_page='"+strcurPageName+"' where elt_account_number=" + strEltAcct + " AND user_id='" + elt_user_id + "'";
                        mCmd.CommandText = Query_login;
                        mCmd.ExecuteNonQuery();
                    }
...
// KAS AMERICA
                case "192.168.0.12":
                    return "server=192.168.0.3;database=DEVDB;user id=sa; password=";
                case "s-app01":
                    return "server=localhost;database=PRDDB;user id=sa; password=dpV8XXVK";
