    ...
    Else
        ...
        If NOT rs.EOF AND NOT rs.BOF Then
            If rs("is_dome") = "Y" Then
                Response.Write("<script> parent.document.frames['topFrame'].changeTopModule('Domestic'); window.location.href='/IFF_MAIN/ASP/domestic/new_edit_hawb.asp?mode=search&HAWB=" & vHAWB & "'; </script>")    
                Response.End()
            End If
            rs.close()
        End If
...
SUB GET_HAWB_INFORMATION_FROM_TABLE
	...
	if Not vHAWB="0" and Not vHAWB="" then
		...
		If Not rs.EOF Then
		    ...
		    rs.Close
		    Response.Write("<script> alert('Could not find the HAWB'); history.go(-1); </script>")
		    Response.End()
		end if
...
<script type="text/jscript"> alert('Could not find the HAWB'); history.go(-1); </script>
...
<head>
   ...
   //ADDef by stanley Limit Fumction
    ...
    function SetCreditNote(){
        ...
        var vWinArg = "dialogWidth:600px; dialogHeight:400px; help:no; status:no; scroll:yes; center:yes";
        var vReturn = showModalDialog("../acct_tasks/credit_note_list.asp?TYPE=A&MAWB=" + encodeURIComponent(document.getElementById("lstMAWB").value)+ "&HAWB=" + encodeURIComponent(document.getElementById("txtHAWB").value), "popWindow", vWinArg);
        
        if(vReturn >= 0){
            try {parent.document.frames['topFrame'].changeTopModule("Accounting");}catch(err){}
            if(vReturn == 0){
                window.location.href = "../acct_tasks/edit_credit_note.asp?new=yes&MoveType=AIR&MasterOnly=N&InvType=Agent&AgentID=" + document.getElementById("hFFAgentAcct").value + "&MAWB=" + encodeURIComponent(document.getElementById("lstMAWB").value)+ "&HAWB=" + encodeURIComponent(document.getElementById("txtHAWB").value);
            }
            else{
    ...
    Sub ScaleChange(ItemNo)
	    ...
	    AW = document.all("AdjustedWeight").item(ItemNo+1).Value
	    DW = document.all("Dimension").item(ItemNo+1).value
	    ...
	    '// Setting memories for exact amount
	    if IsNull(memGW) then
		    memGW=cDbl(GW)
	    end if
	    if IsNull(memCW) then
		    memCW=cDbl(CW)
	    end if
	    if IsNull(memAW) then
		    memAW=cDbl(AW)
	    end if
	    if IsNull(memDW) then
		    memDW=cDbl(DW)
	    end if
	    ...
	    if Scale="K" then
		    ...
		    memAW=memAW / 2.20462262
		    memDW=memDW / 2.20462262
			'//reverse for rate, dont convert when ratecharge is "N/A"
			If RateCharge <> "N/A" Then
	    ...
	    document.all("GrossWeight").item(ItemNo+1).Value= Round(memGW)
	    document.all("ChargeableWeight").item(ItemNo+1).Value = Round(memCW)
	    document.all("AdjustedWeight").item(ItemNo+1).Value = Round(memAW)
    	document.all("Dimension").item(ItemNo+1).value = Round(memDW)
    	
	    If IsNumeric(RateCharge) Then
			document.all("RateCharge").item(ItemNo+1).Value = RateCharge
			document.all("TotalCharge").item(ItemNo+1).Value = 0
        end if 

        Call checkDecimalTextMax(document.all("GrossWeight").item(ItemNo+1),8)
        Call checkDecimalTextMax(document.all("ChargeableWeight").item(ItemNo+1),8)
        Call checkDecimalTextMax(document.all("AdjustedWeight").item(ItemNo+1),8)
	    Call checkDecimalTextMax(document.all("Dimension").item(ItemNo+1),8)
    end Sub

    Sub TopSearch()

        SearchStr=document.frmHAWB.txtSearchStr.Value
        sIndex=document.frmHAWB.lstSearchType.selectedindex
        SearchType=document.frmHAWB.lstSearchType.item(sIndex).Value

        if SearchType="Shipper" then
	        Call SearchOrg("Shipper","lstShipperName","txtSearchStr")
        elseif SearchType="Consignee" then
	        Call SearchOrg("Consignee","lstConsigneeName","txtSearchStr")
        elseif SearchType="Notify" then
	        Call SearchOrg("Notify","lstNotifyName","txtSearchStr")
        elseif SearchType="Agent" then
	        Call SearchOrg("Agent","lstFFAgent","txtSearchStr")
        else
	        Call SearchOrg("Vendor","lstVendor#" & cInt(SearchType)-1,"txtSearchStr")
        end if
    ...
    IF NOT IVstrMSG = "" THEN
	    ok=MsgBox ("Invoice No. " & IVstrMSG & " for HAWB#:" & tvHAWB & " was processed already." & chr(10) & "Do you want to continue?",36,"Warning!")
	    if ok=6 then
		    CHECK_IV_STATUS = true
		    exit function
	    else
    ...
    Function eltMsgBox(Msg)
        returnvalue=MsgBox (Msg,3,"Attention")
        eltMsgBox=returnvalue
    End Function



    Sub CreateMasterHouse
    ...
    /////////////////////////////
    Sub bsaveclick(TranNo,SaveAsNew)
    /////////////////////////////
        
        Dim i, is_copied
        
        // added by Joon on 10-15-2007 new save as options
        If SaveAsNew = "yes" And document.getElementById("lstM_HAWB_Text").value <> "" Then
            is_copied = "yes"
        Else
            is_copied = "no"
        End If    
        ...
        To0=document.frmHAWB.txtTo.Value
        To1=document.frmHAWB.txtTo1.Value
        To2=document.frmHAWB.txtTo2.Value
        ...
        If "<%=vCheckMH%>" = "Y" Then
            ...
            Else 
                Document.frmHAWB.hSubCount.Value = cInt("<%=SHIndex%>")
	        End If 	
        End If 
        ...
        HAWB_NUM=document.frmHAWB.txtHAWB.Value

        if Not SaveAsNew="yes" then

	        ////////////////////////////////////////	
	        IF NOT CHECK_IV_STATUS( HAWB_NUM ) THEN
		        EXIT SUB
	        END IF
	        ////////////////////////////////////////
        	
	        if HAWB_NUM = "" or HAWB_NUM = "0"  then
		        SaveAsNew="yes"
	        end if
        end if
        ...
        pos=instr(HAWB_NUM,"-")
        if pos>0 then
	        HAWB_NUM=Mid(HAWB_NUM,pos+1,20)
        end if
        pos=0
        pos=instr(HAWB_NUM,"-")
        if pos>0 then
        ...
        If Document.frmHAWB.lstFFAgent.value = "" then
	        if document.frmHAWB.hCheckMH.Value="Y"	then
	        else 
		        MsgBox "Please select an agent!"
		        Exit Sub
	        end if 
        Else
	        for i=1 to WC
		        ...
		        elseif Not IsNumeric(document.all("GrossWeight").item(i).Value) then
			        ...
			        Exit Sub
		        elseif Not IsNumeric(document.all("AdjustedWeight").item(i).Value) then
			        MsgBox "Please enter a Numeric Value for ADJUSTED WEIGHT!"
			        Exit Sub
		        elseif Not IsNumeric(document.all("ChargeableWeight").item(i).Value) then
			        ...
			        Exit Sub
		        elseif Not IsNumeric(document.all("Dimension").item(i).Value) then
			        MsgBox "Please enter a Numeric Value for DIMENSIONAL WEIGHT!"
			        Exit Sub
	        ...
	        For i=1 to OC
		        ...
		        If IsNull(oAmt) Or oAmt = ""  Then oAmt = 0
		        if Not IsNumeric(oAmt) then
			        MsgBox "Please enter a Numeric Value for CHARGE AMT!"
			        Exit Sub
		        end if
		        if not oAmt=0 and oItem="0-Select One" then
			        MsgBox "Please select an item!"
			        Exit Sub
		        end if
	        Next
	        if Not document.frmHAWB.txtPrepaidValuationCharge.Value="" and Not IsNumeric(document.frmHAWB.txtPrepaidValuationCharge.Value) then
		        MsgBox "Please enter a Numeric Value for VALUATION CHARGE!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtCollectValuationCharge.Value="" and Not IsNumeric(document.frmHAWB.txtCollectValuationCharge.Value) then
		        MsgBox "Please enter a Numeric Value for VALUATION CHARGE!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtPrepaidTax.Value="" and Not IsNumeric(document.frmHAWB.txtPrepaidTax.Value) then
		        MsgBox "Please enter a Numeric Value for TAX!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtCollectTax.Value="" and Not IsNumeric(document.frmHAWB.txtCollectTax.Value) then
		        MsgBox "Please enter a Numeric Value for TAX!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtConversionRate.Value="" and Not IsNumeric(document.frmHAWB.txtConversionRate.Value) then
		        MsgBox "Please enter a Numeric Value for CONVERSION RATE!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtCCCharge.Value="" and Not IsNumeric(document.frmHAWB.txtCCCharge.Value) then
		        MsgBox "Please enter a Numeric Value for CC CHARGE!"
                Exit Sub
	        end if
	        if Not document.frmHAWB.txtChargeDestination.Value="" and Not IsNumeric(document.frmHAWB.txtChargeDestination.Value) then
		        MsgBox "Please enter a Numeric Value for DESTINATION CHARGE!"
                Exit Sub
	        end if
        		
            NewHAWB=document.frmHAWB.hNewHAWB.value
            HAWBPrefix=document.frmHAWB.hHAWBPrefix.Value
            ...
            If SaveAsNew="yes" then
                ...
                tmpUrl = "new_edit_hawb_OK.asp?SaveAsNew=yes&prefix=" _
                    & encodeURIComponent(HAWBPrefix) & "&NEXTPREFIX=" _
                    & encodeURIComponent(NEXTPrefix) & "&salesPerson=" _
                    & encodeURIComponent(vSalesPerson) & "&IsCopied=" & is_copied
                Set NewNum = showModalDialog("hawb_Dialog.asp?" & tmpUrl ,,"dialogWidth:400px; dialogHeight:170px; help:no; status:no; scroll:no;center:yes")
		        ...
		        If Not NewNum.hawbNum = "" Then
			        document.frmHAWB.hHAWBPrefix.value = ""			
			        document.frmHAWB.action= "new_edit_hawb.asp?save=yes&New=yes&HAWB=" & encodeURIComponent(NewNum.hawbNum) _
		        ...
		        End if
            Else
                If NOT TRIM(HAWB_NUM) = "" Then 
			        NewHAWB = HAWB_NUM // This logic will be processed ( Click Save -> Refresh -> Click Save ) 
		        End If
		        document.frmHAWB.action="new_edit_hawb.asp?save=yes&New=" & encodeURIComponent(NewHAWB) _
		            & "&tNo=" & TranNo & "&WindowName=" & window.name 
		        document.frmHAWB.method="POST"
		        document.frmHAWB.target=window.name
		        frmHAWB.submit()
    ...
    Sub lstMAWBChange()
	    ...
	    if Document.frmHAWB.lstMAWB.item(sindex).text <> "" then
		    ///////////////////////////////////////////////////////////////////////////////////
		    CHECK_INVOICE_STATUS_AJAX "MAWB" , Document.frmHAWB.lstMAWB.item(sindex).text
		    ///////////////////////////////////////////////////////////////////////////////////
	    end if
        ...
        if sindex >=0 then
		    ...
		    if (LCASE(Document.frmHAWB.hmawb_num.Value) = "select one" ) then
            ...
            document.frmHAWB.hAirOrgNum.Value=mAirOrgNum
            document.frmHAWB.hOriginPortID.Value=mDepartureAirportCode
            document.frmHAWB.txtDepartureAirport.Value=mDepartureAirport
            document.frmHAWB.txtTo.Value=mTo
            document.frmHAWB.txtBy.Value=mBy
            document.frmHAWB.txtTo1.Value=mTo1
            document.frmHAWB.txtBy1.Value=mBy1
            document.frmHAWB.txtTo2.Value=mTo2
            document.frmHAWB.txtBy2.Value=mBy2
            document.frmHAWB.txtDestAirport.Value=mDestAirport
            document.frmHAWB.txtFlightDate1.Value=mFlightDate1
            document.frmHAWB.txtFlightDate2.Value=mFlightDate2
            document.frmHAWB.hExportDate.Value=mExportDate
            document.frmHAWB.txtDestCountry.value=mDestCountry
            document.frmHAWB.hDepartureState.value=mDepartureState
			document.frmHAWB.hfDestArrCode.value = mTo2

			If document.frmHAWB.hfDestArrCode.value = "" Then
				document.frmHAWB.hfDestArrCode.value = mTo1
			End If
			If document.frmHAWB.hfDestArrCode.value = "" Then
				document.frmHAWB.hfDestArrCode.value = mTo
			End If
            ...
            if Mid(IssuedBy,1,11) = "AS AGENT OF" then
	            IssuedBy="AS AGENT OF " & mCarrierDesc
            else
	            IssuedBy=IssuedBy & chr(10) & "AS AGENT OF " & mCarrierDesc
            end if

            document.frmHAWB.txtIssuedBy.Value=IssuedBy
            vExecute=document.frmHAWB.hExecution.value

            document.frmHAWB.txtExecute.Value = IssuedBy & chr(10) & vExecute
        end if

    End Sub

    Sub cPPO2Click()
        If document.frmHAWB.cPPO2.checked=FAlse then
    ...
    Sub cCOLL2Click()
        If document.frmHAWB.cCOLL2.checked=FAlse then
	        document.frmHAWB.cCOLL2.VALUE=""
        ...
        End Sub
        Sub cPPO1Click()
        If document.frmHAWB.cPPO1.checked=FAlse then
        ...
        End Sub
        Sub cCOLL1Click()
        If document.frmHAWB.cCOLL1.checked=FAlse then
	        document.frmHAWB.cCOLL1.VALUE=""
        ...
        Sub DeleteWC(ItemNo)
        if document.frmHAWB.hNoItemWC.Value>0 and not cint(document.frmHAWB.hNoItemWC.Value)=ItemNo then
	        ok=MsgBox ("Do you really want to delete this Weight Charge?" & chr(13) & "Please click Yes to continue.",36,"Warning!")
	        if ok=6 then
    ...
    Sub AddOC()
	    NoItem=Cint(document.frmHAWB.hNoItemOC.Value)
	    if NoItem>=10 then
		    MsgBox "Can't have more than 10 Charge Items!"
	    else
		    document.frmHAWB.hNoItemOC.Value=NoItem+1
		    document.frmHAWB.action="new_edit_hawb.asp?AddOC=yes&focus=tblOtherCharge" & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
		    document.frmHAWB.method="POST"
		    document.frmHAWB.target=window.name
		    frmHAWB.submit()
	    end if
    ...
    Sub DeleteOC(ItemNo)
        if document.frmHAWB.hNoItemOC.Value>0 and not cint(document.frmHAWB.hNoItemOC.Value)=ItemNo then
	        ok=MsgBox ("Do you really want to delete this Other Charge?" & chr(13) & "Please click Yes to continue.",36,"Warning!")
	        if ok=6 then
    ...
    //'//////////////////////////////////////////////////
        ...
        Dim Dem,pos,pos1,DD,D1,D2,D3,D4,TotalDem,TemDem,NoPiece,nPiece,Scale,Factor,i
        TotalDem=document.all("Dimension").item(ButtonNum).Value

        if TotalDem="" then TotalDem=0



        TotalCharge=document.all("TOTALCHARGE").item(ButtonNum).value
        RateCharge=document.all("RateCharge").item(ButtonNum).Value

        if RateCharge="N/A" then
	         RateCharge=0
        end if 

        document.all("RateCharge").item(ButtonNum).Value = RateCharge

        if cChanged(ButtonNum-1) <> True then
	        document.all("ChargeableWeight").item(ButtonNum).Value = ""
	        ChargeableWeight=0
        else
	        ChargeableWeight=document.all("ChargeableWeight").item(ButtonNum).Value
        end if

        if ButtonNum<=3 then
	        NoItem=Cint(document.frmHAWB.hNoItemWC.Value)
	        Pieces=document.all("Pieces").item(ButtonNum).Value
	        if Pieces="" then Pieces=0
	        GrossWeight=document.all("GrossWeight").item(ButtonNum).Value
	        AdjustedWeight=document.all("AdjustedWeight").item(ButtonNum).Value
	        if AdjustedWeight="" then
		        document.all("AdjustedWeight").item(ButtonNum).Value=GrossWeight
	        end if
	        if GrossWeight="" then GrossWeight=0
	        RateClass=document.all("RateClass").item(ButtonNum).Value
	        if IsNumeric(Pieces)=false then
		        MsgBox "Please enter a numeric value for PIECES"
	        elseif IsNumeric(GrossWeight)=false then
		        MsgBox "Please enter a numeric value for Gross Weight"
	        elseif not TotalCharge="" and IsNumeric(TotalCharge)=false then
		        MsgBox "Please enter a numeric value for TOTAL CHARGE!"
	        elseif not RateCharge="" AND IsNumeric(RateCharge)=false then
		        if Not Ucase(RateCharge)="MIN" then
			        MsgBox "Please enter a numeric value for Charge Rate"
		        end if
	        elseif GrossWeight=0 then
		        MsgBox "Please enter the GROSS WEIGHT!"
	        elseif cLng(TotalPieces)>0 and not cLng(TotalPieces)=cLng(Pieces) then
		        MsgBox "No of Pieces doesn't match!"
	        else

		        if Round(GrossWeight,0) > Round(TotalDem,0) then
			        TotalDem=GrossWeight
		        end if

		        if ( ChargeableWeight = "" or ChargeableWeight = 0 )  and ( cChanged(ButtonNum-1) <> True ) then
 			        ChargeableWeight=TotalDem
		        end if
              
		        if not RateCharge="" then
        		   
			        if RateCharge>0 then
				        TotalCharge=RateCharge*ChargeableWeight
				        TotalCharge=Round(TotalCharge,2)
			        end if
		        end if

		        document.all("ChargeableWeight").item(ButtonNum).Value=ChargeableWeight
		        document.all("TotalCharge").item(ButtonNum).Value=TotalCharge
		        Pieces=0
		        GrossWeight=0
		        TotalCharge=0
		        for i=1 to NoItem
			        Pieces=Pieces+document.all("Pieces").item(i).Value
			        GrossWeight=GrossWeight+document.all("GrossWeight").item(i).Value
			        if not document.all("TotalCharge").item(i).Value="" then
				        TotalCharge=TotalCharge+document.all("TotalCharge").item(i).Value
			        end if
		        next
		        ...
		        if ButtonNum=NoItem+1 and NoItem<3 then
			        document.frmHAWB.hNoItemWC.Value=NoItem+1
			        document.frmHAWB.action="new_edit_hawb.asp?AddWC=yes&focus=tblWeightCharge" & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
			        document.frmHAWB.method="POST"
					document.frmHAWB.target="_self"
					MsgBox "Please, save the House AWB after updating. "
			        frmHAWB.submit()
		        else
			        document.frmHAWB.action="new_edit_hawb.asp?AddWC=yes&focus=tblWeightCharge" & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
			        document.frmHAWB.method="POST"
	        ...
	        MsgBox "Can't add more than 3 weights!"
        end if
    ...
    /////////////////////////////
        ...
        HAWB=UCASE(document.frmHAWB.txtqHAWB.value)
        document.frmHAWB.txtqHAWB.value = ""
        if not HAWB = "" and not HAWB = "Search Here" then
	        document.frmHAWB.hNewHAWB.value = ""
        else
	        msgbox "Please enter a House AWB No!"
	        exit sub
        end if

        HAWB=Replace(HAWB," ","")
	        document.frmHAWB.action="new_edit_hawb.asp?Edit=yes&hawb=" & encodeURIComponent(HAWB) _
	            & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
	        document.frmHAWB.method="POST"
	        document.frmHAWB.target=window.name
	        frmHAWB.submit()
    ...
    Sub AddToMaster(addSUB,addELTAcct)
	    document.frmHAWB.action="new_edit_hawb.asp?focus=tblSubHouses&addSUB=yes&Edit=yes&addSUBNo=" _
	        & encodeURIComponent(addSUB) & "&addELTAcct=" & addELTAcct & "&hawb=" _
	        & encodeURIComponent("<%=vHAWB%>") & "&MAWB=" & encodeURIComponent("<%=vMAWB%>") _
	        & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
	    document.frmHAWB.method="POST"
	    document.frmHAWB.target = window.name
	    frmHAWB.submit()
    ...
    Sub ChargeItemChange(index)
        ...
        if sindex>0 then
	        ...
	        Document.all("ChargeAmt").item(index+1).value=FormatNumber(ItemUnitPrice,2,,,0)
        else
    ...
    Sub Desc2KeyUp()
        Info=document.frmHAWB.txtDesc2.Value
    ...
    Sub DeleteHAWB()
        HAWB=document.frmHAWB.txtHAWB.Value

        if Not HAWB  = "" then

	        ok=MsgBox ("Do you really want to delete HAWB " & HAWB & "?" & chr(13) & "Please click Yes to continue.",36,"Warning!")
	        if ok=6 then
		        ////////////////////////////////////////	
		        IF NOT CHECK_IV_STATUS( HAWB ) THEN EXIT SUB
		        ////////////////////////////////////////
		        document.frmHAWB.action="new_edit_hawb.asp?DeleteHAWB=yes&HAWB=" & encodeURIComponent(HAWB) _
		            & "&tNo=" & "<%=TranNo%>" & "&WindowName=" & window.name 
		        document.frmHAWB.method="POST"
		        document.frmHAWB.target=window.name
		        frmHAWB.submit()
    ...
    Sub COLOClick()
        if document.frmHAWB.cCOLO.checked=false then
    ...
    function setChargeableWeight(index){
	    ...
	    var gross = document.getElementById(id1).value;	
	    var dimension = document.getElementById(id2).value;
	    if(gross==""){
		    gross="0";
	    }
	    if(dimension==""){
    ...
    function setAdjustedWeight(index){
	    ...
	    var goss = document.getElementById(id1).value;
	    
	    document.getElementById(id2).value = goss;
    }
...
<body link="336699" vlink="336699" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"
    ...
    <form method="post" name="frmHAWB">
                ...
                <tr>
                    ...
                    <td width="55%" align="right" valign="bottom">
                        ...
                        <div id="print">
                            ...
                            <img src="/iff_main/ASP/Images/button_devider.gif" alt="" />
                            <a href="javascript:;" onclick="NewPrintVeiw();return false;" tabindex="-1">
                    ...
                    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td width="48%" align="center" valign="middle">
                                <img src="../images/button_save_medium.gif" name="bSave" width="43" height="18" onclick="if('<%=vCheckSH%>'=='Y'){CheckIfMasterHasSameMAWB();}else{bsaveClick('<%= TranNo %>','no')}"
                                    style="cursor: hand"></td>
                            <td width="13%" align="right" valign="middle">
                                <a href="/iff_main/ASP/air_export/new_edit_hawb.asp" tabindex="-1">
                ...
                <td bgcolor="#f3f3f3" colspan="2">
                    ...
                    <table width="60%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#A0829C"
                        ...
                        <tr bgcolor="#f3f3f3">
                            ...
                            <td width="40%">
                                <span class="bodyheader">
                                    <img src="/iff_main/ASP/Images/required.gif" align="absbottom">Agent</span></td>
                            <td width="19%">
                                &nbsp;
                            </td>
                            <td width="40%">
                                            ...
                                            <div id="<%=iMoonComboBoxName%>_Container" class="ComboBox" style="width: <%=iMoonComboBoxWidth%>;
                                                ...
                                                <span class="ComboBox" style="width: <%=iMoonComboBoxWidth%>; position: ; top: ;
                                                    ...
                                                    <input name="<%=iMoonComboBoxName%>:Text" type="text" id="<%=iMoonComboBoxName%>_Text"
                                                        ...
                                                        autocomplete="off" style="width: <%=iMoonComboBoxWidth%>; vertical-align: middle;
                                                        <% if iscoloaded then response.write("background-color:#cccccc") end if %>" <% if iscoloaded then response.write("readonly") end if%>
                                    ...
                                    <tr>
                                        <td height="32">
                                            <span class="goto"><a href="javascript:;" onclick="MAWBEditClick(); return false;">
                                                <img src="/iff_main/ASP/Images/icon_goto.gif" align="absbottom" />Go to MAWB</a></span></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
            ...
            <tr id="td_wc_list" align="left" valign="middle">
                <td>
                    <input name="txtPiece<%= i %>"  type="text" style="width: 50px; behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="Pieces" value="<%=aPiece(i)%>" size="8"></td>
                ...
                <td>
                    <select name="lstKgLb<%= i %>" size="1" class="smallselect" id="KgLb" onchange="ScaleChange(<%= i %>)">
                ...
                <td>
                    <input name="txtGrossWeight<%= i %>" type="text" style="width: 58px; behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="GrossWeight" value="<%=aGrossWeight(i)%>" size="9" onblur='setChargeableWeight("<%= i %>"); setAdjustedWeight("<%= i %>");' /></td>
                <td>
                    <input name="txtAdjustedWeight<%= i %>" type="text" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="AdjustedWeight" value="<%=aAdjustedWeight(i)%>" size="9" /></td>
                <td>
                    <input name="txtRateClass<%= i %>" type="text" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="RateClass" value="<%= aRateClass(i) %>" size="5" maxlength="1"></td>
                <td>
                    <input name="txtItemNo<%= i %>" type="text" class="numberfield" value="<%= aItemNo(i) %>"
                        maxlength="16" size="7"></td>
                <td>
                    <input name="txtDimension<%= i %>" type="text" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="Dimension" value="<%= aDimension(i) %>" size="7" onblur='setChargeableWeight("<%=i%>")' />
                ...
                <td>
                    <input name="txtChargeableWeight<%= i %>" type="text" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="ChargeableWeight" value="<%= aChargeableWeight(i) %>"
                ...
                <td>
                    <input name="txtRateCharge<%= i %>" type="text" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="RateCharge" value="<%
            ...
            if ConvertAnyValue(aRateCharge(i),"Double",0) = 0  Then 
                ...
                <td <%if vcheckmh="Y" then response.write("style='visibility:visible'")else  response.write("style='visibility:visible'")  %>>
                    <input name="txtTotal<%= i %>" type="text" style="width: 58px; behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="TotalCharge" value="<%=ConvertAnyValue(aTotal(i),"Amount",0) %>"
                        size="10" /></td>
        ...
        <table id="tblOtherCharge" width="95%" border="0" align="center" cellpadding="0"
            ...
            <% for i=0 to oIndex-1 %>
            <tr bgcolor="ffffff" id="aRow<%=i %>" style="visibility: <% if vCheckMH ="Y" then response.Write("visible") end if%>">
                <td>
                ...
                <td>
                    <input type="radio" <% if acollectprepaid(i)="C" then response.write("checked") %>
                        id="ChargeCP" name="lstCollectPrepaid<%= i %>" value="C" />
                    <input type="radio" <% if acollectprepaid(i)="P" then response.write("checked") %>
                        id="ChargeCP" name="lstCollectPrepaid<%= i %>" value="P" /></td>
                <td>
                    <select style="width: 230px" name="lstChargeCode<%= i %>" size="1" class="smallselect"
                        id="ChargeItem" onchange="ChargeItemChange(<%= i %>)">
                ...
                <td>
                    <input name="txtChargeDesc<%= i %>" class="shorttextfield" id="ItemDesc" value="<%= aDesc(i) %>"
                        size="46" /></td>
                <td>
                    <input name="txtChargeAmt<%= i %>" style="behavior: url(../include/igNumDotChkLeft.htc)"
                        class="numberfield" id="ChargeAmt" value="<%= ConvertAnyValue(aChargeAmt(i),"Amount",0) %>"
            ...
            <tr align="right">
                <td height="20" colspan="5" align="left">
                    <span style="visibility: <% if vCheckMH ="Y" then response.Write("visible") end if%>">
                        <input type="image" src="../images/button_addcharge.gif" width="113" height="18"
                            name="bAddOC" onclick="AddOC(); return false;" style="cursor: pointer" /></span></td>
                <!--            
                ...
                <td height="24" colspan="2" bgcolor="E5D4E3">
                    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td width="13%" align="right" valign="middle">
                                <a href="/iff_main/ASP/air_export/new_edit_hawb.asp">
                                    <img src="/iff_main/ASP/Images/button_new.gif" width="42" height="17" border="0"
...
<script type="text/javascript">
    ...
    { 
	    ...
	    try
	    {
	        var url="../ajaxFunctions/ajax_updateAdjustedWeight.asp?MAWB=" + encodeURIComponent(MAWB)
    ...
    {   
	    ...
	    if (document.getElementById("lstShipperName").value == "") {
	        ...
	        if(pos>1){
                lstShipperNameChange(defaultShipper.substring(0,pos-1),defaultShipper.substring(pos+1,1000));
		        document.getElementById	("txtSignature").value = "AS AGENT" + "\n" + "FOR " + defaultShipper.substring(pos+1,1000);	
		    }
	    }
	    return true;
    }
    ...
    { 
	    ...
	    try
	    {
		    xmlHTTP.open("get","../ajaxFunctions/ajax_checkIfMasterHasSameMAWB.asp?MAWB=" 
    ...
    function getCustomerSellingRate(index){
	    ...
	    if (req) {
		    ...
		    req.open("GET","../ajaxFunctions/ajax_cus_rate.asp?cusAcc=" + encodeURIComponent(cusAcc) 
				...
				//window.open("../ajaxFunctions/ajax_cus_rate.asp?cusAcc=" + encodeURIComponent(cusAcc) + "&Unit=" + encodeURIComponent(Unit) + "&airline=" + encodeURIComponent(airline) + "&arp=" + encodeURIComponent(arp) + "&dep=" + encodeURIComponent(dep) + "&wgt=" + encodeURIComponent(wgt));
		    req.send();		
	    }
	    <% End If %>
    }
