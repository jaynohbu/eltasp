...
<head>
    ...
    Sub AddNewItem
        ...
        vNewItemNo = GetSQLResult("SELECT MAX(item_no) FROM item_charge WHERE elt_account_number=" & elt_account_number, Null)
        ...
        Else
            ...
            feData.SetColumnKeys("item_charge")
    ...
    Sub DeleteItem
        ...
        If Not IsItemLocked(deleteID) Then
            SQL = "DELETE FROM item_charge WHERE elt_account_number=" & elt_account_number & " AND item_no=" & deleteID
            eltConn.execute(SQL)
        Else
    ...
    Sub SetupItems
        ...
        SQL = "If NOT EXISTS (SELECT * FROM item_charge WHERE elt_account_number=" & elt_account_number _
            & ") INSERT INTO item_charge (elt_account_number, item_no, item_name, item_type, item_desc, item_def) " _
            & "SELECT '" & elt_account_number & "',item_no,item_name,item_type,item_desc,item_def FROM item_charge AS item_charge_copy where elt_account_number=10001000"
        eltConn.Execute SQL
    End Sub
    
    Function IsItemLocked(vItemCode)
        Dim vRecordCount, SQL
        SQL = "SELECT (SELECT COUNT(charge_code) FROM hawb_other_charge WHERE elt_Account_number=" & elt_account_number & " AND charge_code=" & vItemCode & ")+" _
            & "(SELECT COUNT(charge_code) FROM mawb_other_charge WHERE elt_Account_number=" & elt_account_number & " AND charge_code=" & vItemCode & ")+" _
            & "(SELECT COUNT(charge_code) FROM hbol_other_charge WHERE elt_Account_number=" & elt_account_number & " AND charge_code=" & vItemCode & ")+" _
            & "(SELECT COUNT(charge_code) FROM mbol_other_charge WHERE elt_Account_number=" & elt_account_number & " AND charge_code=" & vItemCode & ")+" _
            & "(SELECT COUNT(item_no) FROM invoice_charge_item WHERE elt_Account_number=" & elt_account_number & " AND item_no=" & vItemCode & ")+" _
            & "(SELECT COUNT(default_air_charge_item) FROM user_profile WHERE elt_Account_number=" & elt_account_number & " AND default_air_charge_item=" & vItemCode & ")+" _
            & "(SELECT COUNT(default_ocean_charge_item) FROM user_profile WHERE elt_Account_number=" & elt_account_number & " AND default_ocean_charge_item=" & vItemCode & ")+" _
            & "(SELECT COUNT(default_domestic_charge_item) FROM user_profile WHERE elt_Account_number=" & elt_account_number & " AND default_domestic_charge_item=" & vItemCode & ")"
    ...
    <script type="text/jscript">
        function AddNewItem(){
            ...
            oForm.action = "charge_items.asp?mode=add";
            oForm.target = "_self";
            oForm.method = "post";
            oForm.submit();
        }
        function DeleteItem(arg){
            if(confirm("Are you sure you want to delete this item? Press OK to contnue.")){
                var oForm = document.getElementById("form1");
                oForm.action = "charge_items.asp?mode=delete&id=" + arg;
                oForm.target = "_self";
                oForm.method = "post";
                oForm.submit();
            }
        ...
        function UpdateItem(i,arg){
            ...
            oForm.action = "charge_items.asp?mode=update&id=" + arg;
            oForm.target = "_self";
            oForm.method = "post";
            oForm.submit();
        }
                    ...
                    <td>
                        <table style="width: 100%" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                ...
                                <td class="bodycopy"><% If checkBlank(aAllData(i)("account_revenue"),"")<>"" And agent_status = "A" Then %>Accounting Enabled<% End If %></td>
                                <td>
