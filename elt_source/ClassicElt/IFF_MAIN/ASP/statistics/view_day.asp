...
<tr>
	<td style="word-break:break-all;padding:5px;color:#ffffff;"><b><% if s_year <> "" then %><%=s_year%>년<% if s_month<>"" then %>&nbsp;<%=s_month%>월<% end if %>의<% else %>전체<%end if %> 날짜별 접속통계</b></td>
</tr>
</table>
<br>
...
<tr align="center">
	...
	<tr><td width="48" style="font-family:Arial;font-size:8pt;color:gray" nowrap align="center"><b>일</b></td></tr>
	</table></td>
	<td width="364">
...
<%
	...
	do while i <= totalDay 
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from statistic where substring(left(convert(nvarchar(20), s_date, 120),10),9,2) = '"&j&"'"
		else	
			if s_month = "" then	'### 년도 전체보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(nvarchar(20), s_date, 120),4) = '"&s_year&"' and substring(left(convert(varchar(20), s_date, 120),10),9,2) = '"&j&"'"
			else	'### 월별보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(nvarchar(20), s_date, 120),7) = '"&s_cal&"' and substring(left(convert(varchar(20), s_date, 120),10),9,2) = '"&j&"'"
			end if
		end if
