...
<% if s_month = "" or s_year = "" then	'### 년도 전체보기 했을시에만 보인다 %>

<table width="484" border="0" cellpadding="0" cellspacing="0" bgcolor="#333333">
<tr>
	<td style="word-break:break-all;padding:5px;color:#ffffff;"><b><% if s_year <> "" then %><%=s_year%>년의<% else %>전체<%end if %> 월별 접속통계</b></td>
</tr>
</table>
<br>
...
<tr align="center">
	...
	<tr><td width="48" style="font-family:Arial;font-size:8pt;color:gray" nowrap align="center"><b>월</b></td></tr>
	</table></td>
	<td width="364">
...
<%
	for i=1 to 12
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from statistic where substring(left(convert(nvarchar(20), s_date, 120),10),6,2) = '"&j&"'"
		else
			SQL="SELECT count(s_date) from statistic where left(convert(nvarchar(20), s_date, 120),4) = '"&s_year&"' and substring(left(convert(varchar(20), s_date, 120),10),6,2) = '"&j&"'"
		end if
