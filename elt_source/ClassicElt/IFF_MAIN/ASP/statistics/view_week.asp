	...
	for i=1 to 7
		...
		if s_year = "" then
			...
			if s_month = "" then	'### 년도 전체보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(nvarchar(20), s_date, 120),4) = '"&s_year&"' and s_week = '"&i&"'"
			else	'### 월별보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(nvarchar(20), s_date, 120),7) = '"&s_cal&"' and s_week = '"&i&"'"
			end if
		end if
