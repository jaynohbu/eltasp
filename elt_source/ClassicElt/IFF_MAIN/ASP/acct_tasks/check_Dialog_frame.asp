...
if ( cType == 'one' )
{
	...
	var j = 1;
			for(i=0; i < oi; i++)
			{
				if ( opener.document.all("BillCheck").item(i+1).checked )
				{	
					if (opener.document.all("hBillNo").item(i+1))
					{
						document.all("hBillNo").item(j).value = opener.document.all("hBillNo").item(i+1).value;
					}
					if (opener.document.all("BillAmtPaid").item(i+1))
					{
						document.all("BillAmt").item(j).value = opener.document.all("BillAmtPaid").item(i+1).value;
					}
					if (opener.document.all("BillMemo").item(i+1))
					{
						document.all("BillMemo").item(j).value = opener.document.all("BillMemo").item(i+1).value;
					}
					j++;
				}
			}
			document.all("detailItem").value = j-1;
			if (opener.document.form1.txtDate) 
			{
				document.all('txtDate').value = opener.document.form1.txtDate.value;
			}
			if (opener.document.form1.txt_print_check_as) 
			{
			document.all('txtVendor').value = opener.document.form1.txt_print_check_as.value;
			}
			if (opener.document.form1.txtVendorInfo) 
			{
			document.all('txtVendorInfo').value = opener.document.form1.txtVendorInfo.value;
			}
			if ( opener.document.form1.txtAmount) 
			{
			document.all('txtAmount').value = opener.document.form1.txtAmount.value;
			}
			if (opener.document.form1.txtMoney) 
			{
			document.all('txtMoney').value = opener.document.form1.txtMoney.value;
			}
			...
			document.modalFrame.location = 'check_pdf.asp?postBack=false&cType=' + '<%=cType%>' + '&oi=' + document.all("detailItem").value;
}
else
{
