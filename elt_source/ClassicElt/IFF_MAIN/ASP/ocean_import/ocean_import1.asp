...
<script type="text/vbscript">
    Sub DeleteClick(MAWB,Sec,AgentELTAcct)
    ok=MsgBox ("Do you really want to delete MAWB " & MAWB & "?" & chr(13) & "Continue?",36,"Message")
    if ok=6 then	
	    document.form1.action=encodeURI("ocean_import1.asp?Delete=yes&MAWB=" & MAWB & "&Sec=" & Sec & "&AgentELTAcct=" & AgentELTAcct   & "&WindowName=" & window.name )
	    Document.form1.target = "_self"
	    document.form1.method="POST"
	    form1.submit()
    end if
    End Sub
    Sub EditClick(MAWB,Sec,AgentELTAcct)
	    iType="<%= iType %>"
	    document.form1.action=encodeURI("ocean_import2.asp?iType=" & iType & "&Edit=yes&MAWB=" & MAWB & "&Sec=" & Sec & "&AgentELTAcct=" & AgentELTAcct  & "&WindowName=" & window.name)
	    Document.form1.target = "_self"
	    document.form1.method="POST"
	    form1.submit()
    End Sub
