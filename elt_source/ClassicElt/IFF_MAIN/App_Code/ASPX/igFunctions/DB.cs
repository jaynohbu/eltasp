...
//////////////////////////////////////////////// view_login check
            ...
            if (mReader.Read())
            {
                ...
                {
                    mReader.Close();
                    Query_login = "Update view_login Set alive = 1, u_time='" + System.DateTime.Now.ToString() + "',requested_page='"+strcurPageName+"' where elt_account_number=" + strEltAcct  + " AND ip='" + System.Web.HttpContext.Current.Request.Cookies["CurrentUserInfo"]["IP"].ToString()  + "' AND server_name='" + System.Web.HttpContext.Current.Request.Cookies["CurrentUserInfo"]["Server_Name"].ToString() + "'";
                    mCmd.CommandText = Query_login;
            ...
            else
            {
                ...
                else
                {
                    ...
                    string script = "<script language='javascript'>";
                    script += "alert('Your session was disconnected by another computer! \\n ('+'" + another_user + ":" + another_ip + "' +')' );";
                    script += "self.close();";
                    if (redPage.Trim() != "")
					{
                    script += "top.location.replace('"+ redPage +"');";
...
////////////////////////////////////////////////
        ...
        {
            return "server=.;database=PRD;user id=sa; password=njy*8824";
            switch (cServerName)
            {
                case "elt":
                    if (cServerPort == "80")
                    ...
                    {
                        return "server=.;database=PRD;user id=sa; password=";
                    }
                ...
                case "w-dev-jp01":
                    ...
                    {
                        return "server=localhost;database=TSTDB;user id=sa; password=dpV8XXVK";
                    }
                    else
                    {
                        return "server=.;database=PRD;user id=sa; password=";
