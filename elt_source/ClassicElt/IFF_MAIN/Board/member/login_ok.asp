...
<%
	...
	SQL1 = "SELECT * FROM view_login where elt_account_number="&elt_account_number&" AND user_id='"&join_id&"'"
...
<!-- //
		ans = confirm("Your ID is being used on " + "<%=another_ip%>" + " already. \n Do you want to refuse that connection ?")
         ...
         { top.location.href='/freighteasy/index.aspx';}
...
'	UPDATESQL = "Update view_login Set alive = 0, user_id ='"&rs("id")&"',user_name='"&rs("name")&"',login_time='"&now&"',u_time='"&now&"' ,requested_page='login_ok.asp' where elt_account_numbder="&elt_account_number&" AND ip='"&session_ip&"'"
