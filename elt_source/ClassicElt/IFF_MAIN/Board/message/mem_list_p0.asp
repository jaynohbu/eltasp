...
<tr>
	...
	sw = Request("sw")
		...
		if sw = "" then
			SQL = "select count(session_id) as recCount from view_login where user_id<>'invalid'"
			...
			SQL = SQL & " WHERE user_id<>'invalid'"
			if int(page) > 1 then
			SQL = SQL & " and session_id not in "
			SQL = SQL & "(SELECT TOP " & ((page - 1) * pagesize) & " session_id FROM view_login"
			SQL = SQL & " where " & ss & " like '%" & sw & "%' and user_id<>'invalid' ORDER BY user_id DESC)"
			end if
			SQL = SQL & " order by session_id desc" 
		
		else
			ss = Request("ss")
					
			SQL = "select count(session_id) from view_login where " & ss & " like '%" & sw & "%' and user_id<>'invalid'"
			Set rs = eltConn.Execute(SQL)
			...
			SQL = "SELECT distinct  TOP " & pagesize & " user_id FROM view_login"
			SQL = SQL & " WHERE " & ss & " like '%" & sw & "%' and user_id<>'invalid'"
			if int(page) > 1 then
			SQL = SQL & " and session_id not in "
			SQL = SQL & "(SELECT TOP " & ((page - 1) * pagesize) & " session_id FROM view_login"
			SQL = SQL & " where " & ss & " like '%" & sw & "%' and user_id<>'invalid' ORDER BY session_id DESC)"
			end if
...
if rs.eof or rs.bof then
	...
	SQL2 = "SELECT * FROM member where id = '"&o_id&"'"
