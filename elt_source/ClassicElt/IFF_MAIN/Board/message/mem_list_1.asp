...
<tr>
	...
	if sw = "" then
		SQL = "select count(num) as recCount from member where elt_account_number=" & elt_account_number & " and UPPER(name)<>'SYSTEM'"
		...
		SQL = SQL & " WHERE elt_account_number = " & elt_account_number & " and UPPER(name)<>'SYSTEM' order by num desc" 
	else
		ss = Request("ss")
				
		SQL = "select count(num) from member where elt_account_number = " & elt_account_number & " AND " & ss & " like '%" & sw & "%' and UPPER(name)<>'SYSTEM' "
		Set rs = db.Execute(SQL)
		...
		SQL = SQL & " and UPPER(name)<>'SYSTEM' order by num desc" 
	end if
L
  Set Rs = db.Execute(SQL)
...
if rs.eof or rs.bof then
	...
	SQL1 = "SELECT * FROM view_login where elt_account_number = " & elt_account_number & " AND user_id='"&rs("id")&"'"
