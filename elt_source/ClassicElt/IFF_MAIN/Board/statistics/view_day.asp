...
<%
	...
	do while i <= totalDay 
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from statistic where substring(left(convert(varchar(20), s_date, 120),10),9,2) = '"&j&"'"
		else	
			if s_month = "" then	'### 년도 전체보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),4) = '"&s_year&"' and substring(left(convert(varchar(20), s_date, 120),10),9,2) = '"&j&"'"
			else	'### 월별보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),7) = '"&s_cal&"' and substring(left(convert(varchar(20), s_date, 120),10),9,2) = '"&j&"'"
			end if
		end if
