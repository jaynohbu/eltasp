...
<%
	for i=0 to 23
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from statistic where substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
		else
			if s_month = "" then	'### 년도 전체보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),4) = '"&s_year&"' and substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
			else	'### 월별보기를 선택했을때 ##
				SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),7) = '"&s_cal&"' and substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
			end if
		end if
