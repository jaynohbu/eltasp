...
<%
	...
	if mode="del_cart" and Request.Form("cart")="" then 
		Response.Redirect "../inc/error.asp?no=3&h_url="&h_url
	end if	
	if mode="del_cart" then
		if session_login_name <> "admin" then Response.Redirect "../inc/error.asp?no=4&h_url="&h_url
	...
	SQL = "SELECT id,filename1,filename2,filename3,filename4,re,resame,reid,mem_auth FROM " & tb & " where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and num = " & num 
    Set rs = db.execute(SQL)
	...
	SQL="SELECT * FROM "&tb&" where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and re="&re&" and resame > "&resame&" and reid > "&reid
	Set rs = db.Execute(SQL)
	
	if not rs.EOF then '현재 지우려는 글에 답변글이 있을경우..

		SQL="SELECT * FROM "&tb&" where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and re="&re&" and resame > "&resame&" and reid = "&reid+1
		Set rs=db.Execute(SQL)
		...
		if not rs.EOF then
			...
			UPDATESQL = UPDATESQL & " where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and num =  " & num
	...
	UPDATESQL = "Update " & tb & " Set reid = reid-1 where  elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and re = " & re & " and reid > " & reid
	db.Execute UPDATESQL
	
	SQL2 = "DELETE FROM " & tb & " where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")&" and num ="&num
	db.Execute SQL2
	...
	if mem_auth > 0 then
		...
		sql = "select count(com_num) from Remarks_comment where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")& " and com_num=" & num
		Set rs= db.Execute(sql)
		...
		SQL2 = "SELECT com_mem_id FROM Remarks_comment where elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")& " and com_num=" & num &" and com_id="&del_j
		Set rs2= db.Execute(sql2)
...
'//////////////////////////////////////////////////////////////////////////////		
		SQL2 = "DELETE FROM Remarks_comment where tb='"&tb&"' and elt_account_number="&elt_account_number&" and org_account_number="&session("vOrgNum")& " and com_num=" & num
		db.Execute SQL2
	...
	Response.Redirect "listRemarks.asp?tb="&tb
%>