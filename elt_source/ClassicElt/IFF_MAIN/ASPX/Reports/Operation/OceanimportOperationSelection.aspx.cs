...
namespace IFF_MAIN.ASPX.Reports.Operation
{
	...
	/// </summary>
	public partial class OceanImportOperationSelection : System.Web.UI.Page
	{
        ...
        protected void iBtnGo_Click(object sender, ImageClickEventArgs e)
        {
           ...
           // Response.Write(Qry);
            ...
            Response.Redirect("OperationResult.aspx?Part=Ocean-Import&" + "WindowName=" + windowName);
		...
		private bool performSearch()
		{
            ...
            string sltWtScale = Request["sltWtScale"];
            string sltMeasureScale = Request["sltMeasureScale"];
            
           // Response.Write(sltWtScale);
           // Response.Write(sltMeasureScale);
          //  Response.End();
            ...
            if (sltWtScale == "KG")
            {
             ...
             // Response.Write(sltMeasureScale);
              // Response.End();
                ...
                QryGrossWeight = " case when (isnull(a.scale1,'KG')='LB') then (isnull(a.gross_wt,0)*0.45359237) Else isnull(a.gross_wt,0) End as [Gross Wt.],";
            }
            //<>'LB' includes null, ocean's scale is normally KG
            else
            {

                QryGrossWeight = " case when (isnull(a.scale1,'KG') <> 'LB') then (isnull(a.gross_wt,0)*2.20462262) Else isnull(a.gross_wt,0) End as [Gross Wt.],";
            }
            if (Request["sltMeasureScale"] == "CFT")
            {
                QryChargeableWeight = " case when(isnull(a.scale2,'CBM')='CBM') then (isnull(a.chg_wt,0)*40) Else isnull(a.chg_wt,0) End as [Chargeable Wt.],";
            }
            else
            {
                QryChargeableWeight = " case when(isnull(a.scale2,'CBM')='CFT') then (isnull(a.chg_wt,0)/40) Else isnull(a.chg_wt,0) End as [Chargeable Wt.],";
            }
            ...
            Qry = @"SELECT 
                ...
                where a.itype='O'and a.elt_account_number = " + elt_account_number +
                " and a.elt_account_number = b.elt_account_number " +
                " and ( a.eta >= '" + timeStart +
                "' and  a.eta  < DATEADD(day, 1,'" + timeEnd +
