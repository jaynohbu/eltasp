...
namespace IFF_MAIN.ASPX.Reports.Accounting
{
	...
	/// </summary>
	public partial class APAgingDetail2 : System.Web.UI.Page
	{
...
//			int iCount = reader.GetSchemaTable().Rows.Count;
			...
			string[,] bField = new string[15,iCount];
			...
			{
				#region // Parent logic
				cName=reader["vendor_name"].ToString().Trim();
				if(cName == "") continue;
...
// (by iMoon...)
                ...
                switch (vBillType)
                {
                    case "G":
                        bField[1, bIndex] = "GJE";
                        bField[14, bIndex] = "../../../ASPX/AccountingTasks/gj_entry.aspx?View=yes&EntryNo=" + bField[4, bIndex].ToString();
                        if (reader["elt_account_number"] != elt_account_number)
                        {
                    ...
                    default:
                        ...
                        bField[14, bIndex] = "../../../ASPX/AccountingTasks/enter_bill.aspx?ViewBill=yes&BillNo=" + bField[3, bIndex].ToString();
                        if (reader["elt_account_number"] != elt_account_number)
                        {
                            bField[14, bIndex] += "&Branch=" + reader["elt_account_number"].ToString() + "&BCustomer=" + cName.Replace("&", "^");
                        }
                        break;
                }
...
//			ds.Tables.Add(dt);
			...
			cdt.Columns.Add(new DataColumn("B/L Date", typeof(string)));
			cdt.Columns.Add(new DataColumn("B/L No.", typeof(string)));
...
//		}
		...
		private void UltraWebGrid1_InitializeRow(object sender, Infragistics.WebUI.UltraWebGrid.RowEventArgs e)
		{
            ...
            if (e.Row.Band.BaseTableName == ChildTable)
            {
                ...
                e.Row.Cells.FromKey("B/L No.").TargetURL = s;
            }
        ...
        protected void UltraWebGrid1_InitializeLayout1(object sender, LayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns.FromKey("Vendor Name").Width = new Unit("200px");
            e.Layout.Bands[0].Columns.FromKey("Type").Width = new Unit("40px");
            e.Layout.Bands[0].Columns.FromKey("B/L Date").Width = new Unit("70px");
            e.Layout.Bands[0].Columns.FromKey("B/L No.").Width = new Unit("40px");
            e.Layout.Bands[0].Columns.FromKey("Ref No.").Width = new Unit("70px");
            e.Layout.Bands[0].Columns.FromKey("Due Date").Width = new Unit("70px");
            ...
            e.Layout.Bands[0].Columns.FromKey("Aging").Footer.Caption = "Total";
            e.Layout.Bands[0].Columns.FromKey("Aging").Footer.Style.HorizontalAlign = HorizontalAlign.Right;
