...
namespace IFF_MAIN.ASPX.OnLines.CompanyConfig
{
	...
	/// </summary>
	public partial class CompanyShowDialog : System.Web.UI.Page
	{
		...
		private void PerformSearchWithNum()
		{
			SqlConnection Con = new SqlConnection(ConnectStr);
			SqlCommand cmd = new SqlCommand("select org_account_number,dba_name,business_address,business_city "
											+ " from organization where elt_account_number=" +elt_account_number+" and org_account_number like '" + strAcctNum+ "%' order by org_account_number",Con);
			SqlDataAdapter Adap = new SqlDataAdapter();
		...
		{		
			...
			if(UltraWebGrid1.Rows.Count<1)
			{
				...
				lblError.Text = "No name was found!";
			}
			else
			{
