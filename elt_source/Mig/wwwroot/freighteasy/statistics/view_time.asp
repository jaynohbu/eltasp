...
<tr align="center">
	...
	<tr>
	  <td width="48" style="font-family:Arial;font-size:8pt;color:gray" nowrap align="center"><b>H</b></td>
	</tr>
	</table></td>
	<td width="364">
	...
	<tr>
	  <td width="68" style="font-family:Arial;font-size:8pt;color:gray" nowrap align="center"><b>con</b></td>
	</tr>
	</table></td>
</tr>
<%
	for i=0 to 23
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from IFF_statistics where substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
		else
			if s_month = "" then	'### 년도 전체보기를 선택했을때 ##
				SQL="SELECT count(s_date) from IFF_statistics where left(convert(varchar(20), s_date, 120),4) = '"&s_year&"' and substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
			else	'### 월별보기를 선택했을때 ##
				SQL="SELECT count(s_date) from IFF_statistics where left(convert(varchar(20), s_date, 120),7) = '"&s_cal&"' and substring(convert(varchar(20), s_date, 120),12,2) = '"&i&"'"
			end if
		end if
