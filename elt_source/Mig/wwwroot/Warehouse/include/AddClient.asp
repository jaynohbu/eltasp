...
dba = Replace(dba,"'","''")
	...
	if 	dba = "" then
		...
		if 	dba = OriginalDbaName then
			...
			SQL = "select TOP 1 dba_name from organization where elt_account_number = " & elt_account_number & " AND dba_name > '" & dba & "' order by dba_name"
			Set rs = eltConn.execute (SQL)
...
dba = Replace(dba,"'","''")
	...
	if 	dba = "" then
		...
		if 	dba = OriginalDbaName then	
			...
			SQL = "select TOP 1 dba_name from organization where elt_account_number = " & elt_account_number & " AND dba_name < '" & dba & "' order by dba_name desc"
			Set rs = eltConn.execute (SQL)
...
& " from organization where elt_account_number = " & elt_account_number & " and UPPER(dba_name) = '" & UCase(tmpS) &"'"
...
& "isnull(z_is_special,'') as z_is_special, "_
& "isnull(is_vendor,'') as is_vendor "_
& " from organization where left(elt_account_number,5) = " & MID(elt_account_number,1,5) & " AND UPPER(dba_name) = '" & UCase(tmpS) &"%'"
	Set rs = eltConn.execute (SQL)
	if NOT rs.eof and NOT rs.bof then
...
var strDbaName = document.all.txtDbaName.value;	
	...
	if(trim(orDab_name) == "" && postBack.toLowerCase() == "false") {
		if( !CHECK_ORGANIZATION_AJAX(strDbaName) )
		{
			...
			var param =  'PostBack=false&Type='+'<%=sType%>'+'&s=' + strDbaName; 
