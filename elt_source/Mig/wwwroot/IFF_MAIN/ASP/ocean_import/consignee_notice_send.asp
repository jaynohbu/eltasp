...
<head>
    ...
    <!--  #INCLUDE FILE="../include/MakePODPDFFile.asp" -->
    <%
        Dim ColoDee
        ...
        Sub MakeEmailPDFSub(arg)
			...
			For i=1 To Request.Form("chkAN").Count
			    Call InsertArrivalNoticeIntoPDF_O("no",vMAWB,"","O",Request.Form("chkAN")(i),"","")
			    aHAWB.Add Request.Form("chkAN")(i)
			Next
			
			For i=1 To Request.Form("chkANFC").Count
			    Call InsertArrivalNoticeIntoPDF_O("yes",vMAWB,"","O",Request.Form("chkANFC")(i),"","")
			    aHAWB.Add Request.Form("chkANFC")(i)
			Next
        ...
        Sub SetOutputPDFFile(vOrgNo)
            If vOrgPDF = "" Then
                vOrgPDF = temp_path & "\Eltdata\" & elt_account_number & "\consigneeemail" & vOrgNo & ".pdf"
                r = PDF.OpenOutputFile(vOrgPDF)
                vRecOrgPDF = "consigneeemail" & vOrgNo & ".pdf"
            End If
        End Sub
        ...
        Sub InitializeMail(arg)
            ...
            oMail.Host = MailHost
            oMail.From = "EArrivalNotice@e-logitech.net"
            oMail.FromName = "OCEAN Import EArrival Notice"
            oMail.IsHTML = True
            ...
            vEmailBody = "<html xmlns='http://www.w3.org/1999/xhtml'><head><meta http-equiv='Content-Type' content='text/html; charset=ISO-88591' />" _
                & "<title>OCEAN IMPORT EARRIVAL NOTICE</title><style type='text/css'><!--.text {font-family: Verdana, Arial, Helvetica, sans-serif;	font-size: 0.85em;}--></style></head>" _
                & "<body><p class='text'><font color='#f39003'><strong>OCEAN IMPORT EARRIVAL NOTICE</strong></font></p><p class='text'>" & Server.HTMLEncode(Request.Form("txtMessage")(arg)) _
        ...
        Sub RecordEmailHistory(arg)
            ...
            rs("screen_name").value = "EArrival Notice"
