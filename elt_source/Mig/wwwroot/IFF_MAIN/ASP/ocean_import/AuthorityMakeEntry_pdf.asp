...
If IsNull(vInvoiceNo) Or Trim(vInvoiceNo)="" Then
    vInvoiceNo = -1
End If
...
'if Not vHAWB="" then
 if true then 
	SQL="select a.sub_mawb1 as sub_master,a.it_number as itno,a.it_date as itdate,a.it_entry_port as itentryport,a.cargo_location as cl,a.sec as sec,a.*,a.process_dt as process_dt_h,a.mawb_num as mawb_num,b.*,isnull(a.dep_port,'') as dep_port_h,isnull(a.arr_port,'') as arr_port_h,a.eta as eta_h,a.etd as etd_h from import_hawb a,import_mawb b where a.elt_account_number=b.elt_account_number and a.elt_account_number=" & elt_account_number & " and a.iType=b.iType and a.iType='" & iType & "' and a.mawb_num=b.mawb_num and a.hawb_num='" & vHAWB & "' and a.sec=b.sec and a.invoice_no=" & vInvoiceNo

	'SQL="select a.sub_mawb1 as sub_master,a.it_number as itno,a.it_date as itdate,a.it_entry_port as itentryport,a.cargo_location as cl,a.sec as sec,a.*,a.process_dt as process_dt_h,a.mawb_num as mawb_num,b.* from import_hawb a,import_mawb b where a.elt_account_number=b.elt_account_number and a.elt_account_number=" & elt_account_number & " and a.iType=b.iType and a.iType='" & iType & "' and a.mawb_num=b.mawb_num and a.hawb_num='" & vHAWB & "' and a.sec=b.sec order by sec desc"
...
'	'	vExportAgentName=rs("export_agent_name")
		...
		vFileNo=rs("file_no")
		vFLTNo=checkBlank(rs("flt_no"),rs("vessel_name"))
		vVoyageNo=rs("voyage_no")'<--------------------------------------------------------------from DB
		vProcessDT=rs("process_dt_h")
		vMAWB=rs("mawb_num")
		vPreparedBy=rs("prepared_by")
		vSubMAWB=rs("sub_master")
...
Dim CustomerForm
CustomerForm=oFile & "/Customer/" & "AuthorityMakeEntry_o" & elt_account_number & ".pdf"
...
Else
'// Normal Form
	r = PDF.OpenInputFile(oFile+"/AuthorityMakeEntry_o.pdf")
End If
Set fso = nothing
'////////////////////////////////////////////////////////////////////////
...
PDF.SetFormFieldData "FLTNo",vFLTNo&" / "&vVoyageNo,0'<-------------------------------------------Concat with vVoyageNo
...
PDF.SetFormFieldData "AMS",vSubMAWB2,0
...
If Left(elt_account_number,4) = "2001" Then
    ...
    lastLine = "We, " & vAgentName & ", the consignee of the above mentioned airway bill covering " _
        ...
        & Left(vConsigneeInfo,InStr(vConsigneeInfo,chr(13))-1) _
        & " to make customs entry for " _
...
response.AddHeader "Content-Disposition", "attachment; filename=AM" & Session.SessionID & ".pdf"
