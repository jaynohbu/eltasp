...
Dim aOtherCharge(5),aOtherChargeDesc(5)
Dim hOtherCharge(5),hOtherChargeDesc(5)
...
if Not COLODee="" then
	...
	if rsHAWB.EOF then
		...
		rsHAWB.close
		COPY="CONSIGNEE"
		SQL= "select * from hawb_master where elt_account_number = " & COLODee & " and HAWB_NUM='" & vHAWB & "' and is_dome='Y'"
	end if
else
	SQL= "select * from hawb_master where elt_account_number = " & elt_account_number & " and HAWB_NUM='" & vHAWB & "' and is_dome='Y'"
end if
rsHAWB.Open SQL, eltConn, , , adCmdText
...
Function GetFileNumber()
    ...
    SQL = "SELECT File# from mawb_number where elt_account_number=" & elt_account_number _
        & " AND mawb_no='" & vMAWB2 & "' and is_dome='Y'"
    resVal = ""    
