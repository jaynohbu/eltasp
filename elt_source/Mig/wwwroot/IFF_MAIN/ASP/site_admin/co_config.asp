...
<head>
	...
	function GoCountrySet() {
		var sURL = "../../aspx/OnLines/Country/countryMaster.aspx?WindowName=PopWin";
		viewPop(sURL);
	}
    ...
    function GetOtherCurrency(thisObj,countryObjId){
        ...
        var vURL = "/IFF_MAIN/ASP/site_admin/select_currency.asp?code=" + thisObj.value + "&ccode=" + vCountryCode;
    ...
    function GetOtherCountry(thisObj){
        var vURL = "/IFF_MAIN/ASP/site_admin/select_country.asp?code=" + thisObj.value;
        var vWinArg = "dialogWidth:370px; dialogHeight:280px; help:no; status:no; scroll:no; center:yes";
...
Sub SaveClick()
DIM AccountInfoLen
AccountInfoLen = len(document.form1.txtBcity.value) + len(document.form1.lstBstate.value) + len(document.form1.txtBzip.value)+ len(document.form1.lstBCountry.value)
if ( AccountInfoLen > 40 ) then
	msgbox "The total length of City,State,Zip,Country is too long (" & AccountInfoLen & ")" & chr(13) & " It could not be printed at dot metrix printer."
	exit sub
end if
'if len(document.form1.txtBPhone.value)>11 or len(document.form1.txtCPhone.value)>11 then
'	MsgBox "Phone number must be no more than 11 characters!"
'	exit Sub
'end if
