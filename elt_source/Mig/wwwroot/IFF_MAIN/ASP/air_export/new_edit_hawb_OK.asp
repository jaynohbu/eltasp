...
Sub OkClick
    ...
    if pos>0 then
	    tmpHAWB=Mid(vHAWB,pos+1,20)
	    if len(tmpHAWB)>9 then
	        MsgBox "The To HAWB NO has less then 10 characters!"
	     exit sub	
	     end if
    end if

    if already_e = "True" then
     if errHAWB = vHAWB then
    ...
    document.frmHAWB.action="new_edit_hawb_ok.asp?save=yes&prefix=" & encodeURIComponent(HAWBPrefix) _
        & "&HAWB=" & encodeURIComponent(vHAWB) & "&NEXTPREFIX=" & encodeURIComponent(NEXTPrefix) 
...
Sub PrefixChange()
    ...
    NEXTPrefix= document.frmHAWB.lstHAWBPrefix.item(sindex).value
    vHAWB = ""
    document.frmHAWB.action="new_edit_hawb_ok.asp?SaveAsNew=yes&prefix=" & encodeURIComponent(HAWBPrefix) _
        & "&HAWB=" & encodeURIComponent(vHAWB) & "&NEXTPREFIX=" & encodeURIComponent(NEXTPrefix) 
    document.frmHAWB.method="POST"
    document.frmHAWB.target="_self"
    frmHAWB.submit()
