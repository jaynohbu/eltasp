...
<form name="inno" method="POST" action="find_center_ne.asp" ID="form1">
...
<%
	dim name,email,sql1,rs1
	
	name = request.Form("name")
	email = request.Form("email")

	SQL = "SELECT id,name,email FROM member where elt_account_number=" & elt_account_number & "AND name='"&name&"' and email='"&email&"'"
...
%>
<form name="inno" method="POST" action="find_center_ne.asp" ID="Form2">
...
<%
	...
	SQL = SQL & " where name='"&name&"' and email='"&email&"'"
