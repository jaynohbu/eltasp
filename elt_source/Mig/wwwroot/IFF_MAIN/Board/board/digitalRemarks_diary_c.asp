...
{ 
	...
	function show_layer(num, name, email, url, tb, mem_auth, mem_id, admin) {
		...
		if(email) {
			printMain = "<tr onMouseOver=this.style.backgroundColor='#bbbbbb' onMouseOut=this.style.backgroundColor='' onMousedown=location.href='email.asp?email1="+email+"&h_url=<%=h_url%>';><td style=font-family:verdana;font-size:9pt height=18 nowrap>&nbsp;<img src=../img/n_mail.gif border=0 align=absmiddle>&nbsp;&nbsp;Send eMail&nbsp;&nbsp;</td></tr>";
			printHeight = printHeight + 16;
		}
		if(url) {
			printMain = printMain +	"<tr onMouseOver=this.style.backgroundColor='#bbbbbb' onMouseOut=this.style.backgroundColor='' onMousedown=window.open('http://"+url+"');><td style=font-family:verdana;font-size:9pt height=18 nowrap>&nbsp;<img src=../img/n_homepage.gif border=0 align=absmiddle>&nbsp;&nbsp;홈페이지&nbsp;&nbsp;</td></tr>";
			printHeight = printHeight + 16;
		}
		{
		...
		if(mem_auth > 0) {
			printMain = printMain +	"<tr onMouseOver=this.style.backgroundColor='#bbbbbb' onMouseOut=this.style.backgroundColor='' onMousedown=OpenWindow('../member/user_view.asp?id="+mem_id+"','500','520');><td style=font-family:verdana;font-size:9pt height=18 nowrap>&nbsp;<img src=../img/n_information.gif border=0 align=absmiddle>&nbsp;&nbsp;View User Info.&nbsp;&nbsp;</td></tr>";
			printHeight = printHeight + 16;
			printMain = printMain +	"<tr onMouseOver=this.style.backgroundColor='#bbbbbb' onMouseOut=this.style.backgroundColor='' onMousedown=OpenWindow('../message/m_write.asp?r_id="+mem_id+"&r_name="+name+"','400','400');><td style=font-family:verdana;font-size:9pt height=18 nowrap>&nbsp;<img src=../img/n_msg.gif border=0 align=absmiddle>&nbsp;&nbsp;Send Memo&nbsp;&nbsp;</td></tr>";
			printHeight = printHeight + 16;
			if(admin) {
			printMain = printMain +	"<tr onMouseOver=this.style.backgroundColor='#bbbbbb' onMouseOut=this.style.backgroundColor='' onMousedown=OpenWindow('../member/user_edit.asp?id="+mem_id+"','500','520');><td style=font-family:verdana;font-size:9pt height=18 nowrap>&nbsp;<img src=../img/n_modify.gif border=0 align=absmiddle>&nbsp;&nbsp;<font color=red>User Info. Update&nbsp;&nbsp;</td></tr>";
			printHeight = printHeight + 16;
			}
		}
...
<tr>
	...
	<% if board_type > 0 and view_img = 1 then %><% if i_width1 > 0 and i_height1 > 0 then %><a href="javascript:OpenWindow('../board/open_img.asp?img_file=<%=vDest%><%=filename1%>','<%=rs("i_width1")+16%>','<%=rs("i_height1")+4%>')"><img src="<%=vDest%><%=filename1%>" width="<%=i_width1%>" height="<%=i_height1%>" border="0"></a><br><br><% end if %><% if i_width2 > 0 and i_height2 > 0 then %><a href="javascript:OpenWindow('../board/open_img.asp?img_file=<%=vDest%><%=filename2%>','<%=rs("i_width2")+16%>','<%=rs("i_height2")+4%>')"><img src="<%=vDest%><%=filename2%>" width="<%=i_width2%>" height="<%=i_height2%>" border="0"></a><br><br><% end if %><% if i_width3 > 0 and i_height3 > 0 then %><a href="javascript:OpenWindow('../board/open_img.asp?img_file=<%=vDest%><%=filename3%>','<%=rs("i_width3")+16%>','<%=rs("i_height3")+4%>')"><img src="<%=vDest%><%=filename3%>" width="<%=i_width3%>" height="<%=i_height3%>" border="0"></a><br><br><% end if %><% if i_width4 > 0 and i_height4 > 0 then %><a href="javascript:OpenWindow('../board/open_img.asp?img_file=<%=vDest%><%=filename4%>','<%=rs("i_width4")+16%>','<%=rs("i_height4")+4%>')"><img src="<%=vDest%><%=filename4%>" width="<%=i_width4%>" height="<%=i_height4%>" border="0"></a><br><% end if %><br><% end if %>
	<% dim filetext
	
		'## 동영상 파일 확장자를 적어주세요. 아래의 목록에 있는 파일만 미리보기가 가능합니다.
		filetext="mp3,asf,wmv,mpg,avi,wav"
	 %>
	 <% if board_type > 0 and view_multi = 1 then %><% if filename1 <> "" and Instr(filetext,Lcase(right(filename1,3))) > 0 then %>+ Click the play button .<br>
	 <b>Attached #1</b> : <%=filename1%><br><embed src="<%=vDest%><%=filename1%>" autostart="false"><br><% end if %><% if filename2 <> "" and Instr(filetext,Lcase(right(filename2,3))) > 0 then %><br><b>Attached #2</b> : <%=filename2%><br><embed src="<%=vDest%><%=filename2%>" autostart="false"><br><% end if %><% if filename3 <> "" and Instr(filetext,Lcase(right(filename3,3))) > 0 then %><br><b>Attached #3</b> : <%=filename3%><br><embed src="<%=vDest%><%=filename3%>" autostart="false"><br><% end if %><% if filename4 <> "" and Instr(filetext,Lcase(right(filename4,3))) > 0 then %><br><b>Attached #4</b> : <%=filename4%><br><embed src="<%=vDest%><%=filename4%>" autostart="false"><br><% end if %><% end if %>
	<div align="right" style="font-family:verdana,verdana;font-size:7.9pt;text-align:right;color:#666666;"><img src="../img/<%=title1%>.gif"> <%=writeday%><% if use_reco = 1 then %>
	, &nbsp; <b>Recommended : </b><%=reco%><% end if %><% if view_ip = 1 then %>, &nbsp; <b>IP : </b><%=ip%><% end if %></div><br><%=content%><br><br>	
	</td>
...
%>
<tr>
	<td colspan="2" class="font1" style="word-break:break-all;padding:5px;"><b><span onMousedown="inno_layer('info_<%=com_id%>_<%=num%>','visible')" style=cursor:hand><%=com_name%></span></b> &nbsp; &nbsp; <span style="font-size:7.5pt;"><%=com_writeday%><% if view_ip=1 then %>, &nbsp; IP:<%=com_ip%><% end if %>
	</span>
	<% if com_mem_auth = 0 or (com_mem_auth = 1 and session_login_name = com_mem_id) or session_admin = "admin" then %> &nbsp; &nbsp; <a href="<% if (com_mem_auth=1 and session_login_name = com_mem_id) or session_admin = "admin" then %>memoRemarks_ok<% else %>pin<% end if %>.asp?tb=<%=tb%>&mode=com_del&com_id=<%=com_id%><% if (com_mem_auth=1 and session_login_name = com_mem_id) or session_admin = "admin" then %>&mem=ok<% end if %>&h_url=<%=h_url%>"><img src="../img/but_del_mini.gif" border="0"></a><% end if %></td>
</tr>
<tr>
	<td colspan="2" style="word-break:break-all;padding:5px;font-family:verdana,verdana;font-size:9pt;color:#666666;" valign="top"><%=com_memo%></td>
</tr>
<tr>
	<td colspan="2" height="1" bgcolor="#cccccc"><script>show_layer('info_<%=com_id%>_<%=num%>', '<%=com_rs("com_name")%>', '', '', '<%=tb%>', '<%=com_mem_auth%>', '<%=com_mem_id%>', '<% if session_admin="admin" then %>1<% end if %>');</script></td>
</tr>
...
//-->
	...
	<tr align="center" height="30">
		...
		<tr>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td class="font1"><b>Memo</b></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td><textarea name="com_memo" style="width:100%;" rows="3" class="form_textarea"></textarea>&nbsp;&nbsp;</td>
				<td width="70" align="center" valign="top" style="padding-top:2;"><input type="button" name="com_inno" class="but" value=" O K " style="width:60;height=48" onClick="javascript:ok();"></td>
			</tr>
			</table>
			</td>
		</tr>
...
<table width="<%=board_size%>" border="0" cellpadding="0" cellspacing="0">
<tr>
	<td align="left" style="word-break:break-all;padding:5px;"><% if use_reco = 1 then %><a href="view.asp?tb=<%=tb%>&page=<%=page%>&num=<%=num%>&mode=reco<% if sw<>"" then %>&st=<%=Request.QueryString("st")%>&sc=<%=Request.QueryString("sc")%>&sn=<%=Request.QueryString("sn")%>&sw=<%=Request.QueryString("sw")%><% end if %>"><img src="../img/but_reco.gif" border="0"></a> <% end if %><%if rw_level >= session_level then%><% if (board_type < 2 and notice <> 1) or board_type < 2 then %><a href="formRemarks.asp?tb=<%=tb%>&page=<%=page%>&num=<%=num%>&mode=reply<% if sw<>"" then %>&st=<%=Request.QueryString("st")%>&sc=<%=Request.QueryString("sc")%>&sn=<%=Request.QueryString("sn")%>&sw=<%=Request.QueryString("sw")%><% end if %>"><img src="../img/but_reply.gif" border="0"></a> <% end if %><% end if %><% if w_level >= session("level") then %><a href="formRemarks.asp?tb=<%=tb%>"><img src="../img/but_write.gif" border="0"></a> <% end if %>
	<% if mem_auth = 0 or (mem_auth = 1 and session_uid = id) or session_admin = "admin" then %> <a href="<% if (mem_auth = 1 and session_uid = id) or session_admin = "admin" then %>formRemarks<% else %>pin<% end if %>.asp?tb=<%=tb%>&page=<%=page%>&num=<%=num%>&mode=edit<% if (mem_auth = 1 and session_uid = id) or session_admin = "admin" then %>&mem=ok<% end if %><% if sw<>"" then %>&st=<%=Request.QueryString("st")%>&sc=<%=Request.QueryString("sc")%>&sn=<%=Request.QueryString("sn")%>&sw=<%=Request.QueryString("sw")%><% end if %>"><img src="../img/but_edit.gif" border="0"></a> <a href="<% if (mem_auth = 1 and session_uid = id) or session_admin = "admin" then %>delRemarks_ok<% else %>pin<% end if %>.asp?tb=<%=tb%>&num=<%=num%>&mode=del<% if (mem_auth = 1 and session_uid = id) or session_admin = "admin" then %>&mem=ok<% end if %><% if sw<>"" then %>&st=<%=Request.QueryString("st")%>&sc=<%=Request.QueryString("sc")%>&sn=<%=Request.QueryString("sn")%>&sw=<%=Request.QueryString("sw")%><% end if %>"><img src="../img/but_del.gif" border="0"></a> <% end if %><% if sw<>"" then %><a href="listRemarks.asp?tb=<%=tb%>&page=<%=page%>&st=<%=st%>&sc=<%=sc%>&sn=<%=sn%>&sw=<%=sw%>"><img src="../img/but_search_list.gif" border="0"></a> <% end if %><a href="listRemarks.asp?tb=<%=tb%>&page=<%=page%>"><img src="../img/but_list.gif" border="0"></a></td>
...
%>
	<% If p_num <> 0 Then %><a href="viewRemarks.asp?tb=<%=tb%>&num=<%=p_num%>&page=<%=page%>&sw=<%=sw%>&sn=<%=sn%>&st=<%=st%>&sc=<%=sc%>"><img src="../img/but_pre.gif" border="0"></a><% end if %>&nbsp;<% If n_num <> 0 Then %><a href="viewRemarks.asp?tb=<%=tb%>&num=<%=n_num%>&page=<%=page%>&sw=<%=sw%>&sn=<%=sn%>&st=<%=st%>&sc=<%=sc%>"><img src="../img/but_next.gif" border="0"></a><% end if %>
