...
namespace IFF_MAIN.ASPX.Reports.Accounting
{
    ...
    public partial class ARAgingSelection : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                this.Webdatetimeedit2.Value = DateTime.Now;
                PerformBranch();
            }
        }
...
ON (a.elt_account_number = c.elt_account_number and a.customer_number = c.customer_no) 
WHERE a.elt_account_number=" + elt_account_number + @" AND (invoice_date < DATEADD(day, 1,'" + Webdatetimeedit2.Date.ToShortDateString() + @"')) 
...
ON (a.elt_Account_number=b.elt_account_number AND a.org_Account_number=b.customer_no AND ISNULL(b.credit,0)<>0)
LEFT OUTER JOIN (SELECT * FROM invoice WHERE elt_Account_number=" + elt_account_number + @" AND balance<>0 AND pay_status='A')  c 
ON (a.elt_Account_number=c.elt_Account_number AND a.org_account_number IS NULL) 
WHERE a.elt_account_number=" + elt_account_number + @" AND invoice_no IS NULL";
                if (strCompany != "")
                {
...
SET @selected_date = DATEADD(day,0,'" + Webdatetimeedit2.Date.ToShortDateString() + @"')
...
LEFT OUTER JOIN customer_credits c 
ON (a.elt_account_number = c.elt_account_number and a.customer_number = c.customer_no) 
WHERE a.elt_account_number=" + elt_account_number.Substring(0, 5) + @" AND (invoice_date < DATEADD(day, 1,'" + Webdatetimeedit2.Date.ToShortDateString() + @"')) 
AND balance<>0 AND not isnull(a.customer_name,'1') = '1'";
                if (strCompany != "")
                {
                    strCommandText += " AND a.customer_number=" + strCompany;
...
FROM organization a RIGHT OUTER JOIN customer_Credits b 
ON (a.elt_Account_number=b.elt_account_number AND a.org_Account_number=b.customer_no AND ISNULL(b.credit,0)<>0)
LEFT OUTER JOIN (SELECT * FROM invoice WHERE elt_Account_number=" + elt_account_number.Substring(0, 5) + @" AND balance<>0 AND pay_status='A')  c 
ON (a.elt_Account_number=c.elt_Account_number AND a.org_account_number IS NULL) 
WHERE a.elt_account_number=" + elt_account_number.Substring(0, 5) + @" AND invoice_no IS NULL";
                if (strCompany != "")
                {
                    strCommandText += " AND a.org_Account_number=" + strCompany;
                }
...
END ";
            ...
            str[0] = string.Format("Invoice Date  :  ~ {0}", Webdatetimeedit2.Date.ToShortDateString());
            str[1] = string.Format("Branch Name : {0}", strlblBranch);

            Session["AsOf"] = Webdatetimeedit2.Date.ToShortDateString();
