...
namespace IFF_MAIN.Authentication
{
    ...
    public partial class Authentication : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            // displaying logo for elt client login
            PerformLogoView();
        ...
        private void PerformLogoView()
        {
            ...
            string strFilePath, logoUrl = "";

            strFilePath = ServerUrl + txtClient.Text + ".gif";

            if (File.Exists(strFilePath))
            {
                logoUrl = "/IFF_MAIN/ClientLogos/" + txtClient.Text + ".gif";
            }
            else
            {
                strFilePath = ServerUrl + txtClient.Text + ".jpg";
                if (File.Exists(strFilePath))
                {
                    logoUrl = "/IFF_MAIN/ClientLogos/" + txtClient.Text + ".jpg";
                }
            }

            if (logoUrl == "")
            {
                imgClient_logo.Visible = false;
            }
            else
            {
                imgClient_logo.ImageUrl = logoUrl;
            }
        }
        ...
        private bool performCreateSystemUser(string elt_acct,string phy_path)
        {
            ...
            Response.Cookies["CurrentUserInfo"]["Server_Name"] = this.txtServerName.Text.Trim();
            Response.Cookies["CurrentUserInfo"]["Session_ID"] = Session.SessionID.ToString();
            string redPage = "/IFF_MAIN/Authentication/login.aspx";
            Response.Cookies["CurrentUserInfo"]["ORIGINPAGE"] = redPage;
