...
function myRadioButtonforDateSet3CheckDate(r) {

		myRadioButtonforDateSet3setMyDate(r.value);
		return false;
}

function myRadioButtonforDateSet3setMyDate(r) {

var	Wedit1 = igedit_getById('Webdatetimeedit5')
var	Wedit2 = igedit_getById('Webdatetimeedit6')
...
date = now.getDate(); 	
				if(r== 'Select' ) { 
					...
					tq=month;
					if (tq>=1 && tq<=3) {
						fromDate=new Date(year-1,10-1,1);
						toDate = new Date(year-1,12-1,31);
						}
					else if (tq>=4 && tq<=6 ) {
						fromDate=new Date(year-1,1-1,1);
						toDate = new Date(year-1,3-1,31);
						}
					else if (tq>=7 && tq<=9 ) {
						fromDate=new Date(year-1,4-1,1);
						toDate = new Date(year-1,6-1,30);
						}
					else { 
						fromDate=new Date(year-1,7-1,1);
						toDate = new Date(year-1,9-1,30);
						}
