...
//		string sDetailName = "SEARCHINVOICEDETAIL";
		...
		private void PerformSearch()
		{
			...
			if( Webdatetimeedit1.Text != "" )
			{
				...
				{
					sbSQLWhere.Append(" (invoice_date >= '" + DateTime.Parse(Webdatetimeedit1.Text).ToShortDateString()+ "' "); 
					sbSQLWhere.Append(" AND");
                    sbSQLWhere.Append(" invoice_date < DATEADD(day, 1,'" + DateTime.Parse(Webdatetimeedit1.Text).ToShortDateString() + "')) "); 
					boolAND = true;

                    str[0] = string.Format("Invoice Date : {0}", DateTime.Parse(Webdatetimeedit1.Text).ToShortDateString());
				}
				else
				{
                    sbSQLWhere.Append(" (invoice_date >= '" + DateTime.Parse(Webdatetimeedit1.Text).ToShortDateString() + "' "); 
                    ...
                    str[0] = string.Format("Invoice Date : {0} ~ {1}", DateTime.Parse(Webdatetimeedit1.Text).ToShortDateString(),
                        DateTime.Parse(Webdatetimeedit2.Text).ToShortDateString());
