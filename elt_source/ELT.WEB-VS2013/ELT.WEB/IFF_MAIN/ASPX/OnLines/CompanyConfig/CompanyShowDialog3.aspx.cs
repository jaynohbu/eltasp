...
namespace IFF_MAIN.ASPX.OnLines.CompanyConfig
{
	...
	/// </summary>
	public partial class CompanyShowDialog3 : System.Web.UI.Page
	{
		...
		private void PerformSearch()
		{
			...
			SqlCommand cmd = new SqlCommand("select org_account_number,dba_name,business_address,business_city "
                + " from organization where elt_account_number=" + elt_account_number + " AND ltrim(LOWER(dba_name)) like ltrim(LOWER(substring('" + strDbaName + "',1,10)+'%')) AND org_account_number <> '" + strAcctNum + "' order by dba_name", Con);
		...
		{		
			...
			{
                lblError.Text = "Search results: " + (UltraWebGrid1.Rows.Count).ToString() + " " + " found";
			}
...
//			e.Row.Cells.FromKey("org_account_number").TargetURL = s + e.Row.Cells.FromKey("org_account_number").Value + ")";
        ...
        protected void UltraWebGrid1_InitializeLayout1(object sender, LayoutEventArgs e)
        {
            ...
            UltraWebGrid1.Bands[0].Columns.FromKey("dba_name").Width = new Unit("230px");
            ...
            UltraWebGrid1.Bands[0].Columns.FromKey("org_account_number").Header.Caption = "Num.";
