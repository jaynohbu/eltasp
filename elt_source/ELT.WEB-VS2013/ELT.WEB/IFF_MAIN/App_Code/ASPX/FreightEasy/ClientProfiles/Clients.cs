...
namespace FreightEasy.ClientProfiles
{
    ...
    public class Clients : FreightEasyData
    {
        ...
        public Clients GetClientsByType(string elt_account_number, string bizType)
        {
            ...
            if (bizType == "Shippers")
            {
                clientSQL = "(SELECT dba_name,elt_account_number AS org_account_number FROM agent WHERE elt_account_number="
                    + elt_account_number + ")UNION (SELECT dba_name, org_account_number FROM organization WHERE elt_account_number=" + elt_account_number
