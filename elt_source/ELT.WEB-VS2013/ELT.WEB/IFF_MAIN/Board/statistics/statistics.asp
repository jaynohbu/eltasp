...
<tr>
	<td colspan="<% if s_year="00" then %>7<% else %>6<% end if %>"><img src="../img/statistics_title.gif"></td>
</tr>
...
<%
	SQL="SELECT left(convert(varchar(20), s_date, 120),4) from statistic group by left(convert(varchar(20), s_date, 120),4) order by left(convert(varchar(20), s_date, 120),4) asc"
...
%>
	...
	set rs = nothing
		...
		SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),4) = '"&s_year&"'"
		Set rs = db.Execute(SQL)				if s_month <> "" then			total_year = FormatNumber(rs(0),0)						rs.close			set rs = nothing
			
			SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),7) = '"&s_cal&"'"
