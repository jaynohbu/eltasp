...
<%
	for i=1 to 12
		...
		if s_year = "" then
			SQL="SELECT count(s_date) from statistic where substring(left(convert(varchar(20), s_date, 120),10),6,2) = '"&j&"'"
		else
			SQL="SELECT count(s_date) from statistic where left(convert(varchar(20), s_date, 120),4) = '"&s_year&"' and substring(left(convert(varchar(20), s_date, 120),10),6,2) = '"&j&"'"
		end if
