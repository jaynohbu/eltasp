...
<%
	...
	if mode = "" and w_level < session_level then Response.Redirect "../inc/error.asp?no=4"&h_url

		
	If mode = "reply" Then

		if rw_level < session_level then Response.Redirect "../inc/error.asp?no=4&h_url="&h_url
	...
	If mode = "edit" Then
		...
		if session_uid <> "" and Request.QueryString("mem") = "ok" then
...
<tr bgcolor="F7F7F7" height="25"> 
	<td width="110" class="form_title" align="right"><b>&nbsp;Name&nbsp;</b></td>
	<td width="175"><input type="text" name="name" size="15" class="form_input" maxlength="30" value="<% if session_login_name = "" and mode="edit" then %><%=name%><% else %><% if session_login_name<>"" then %><%=session_login_name%><% else %><% if use_cookies=1 then %><%=Request.Cookies("inno")("name")%><% end if %><% end if %><% end if %>"></td>
	<td width="110" class="form_title" align="right"><b>Password&nbsp;</b></td>
	<td width="175"><input type="password" name="pin" size="15" maxlength="20" class="form_input" value="<% if session_uid = "" and mode="edit" then %><% else %><%=session_pin%><% end if %>"></td>
</tr>
<tr>
	<td colspan="4" height="1" bgcolor="#cccccc"></td>
</tr>
<tr bgcolor="F7F7F7" height="25"> 
	<td width="110" class="form_title" align="right">Email&nbsp;</td>
	<td width="460" colspan="3"><input type="text" name="email" size="30" class="form_input" maxlength="50" value="<% if session_login_name = "" and mode="edit" then %><%=email%><% else %><% if session_login_name<>"" then %><%=session_email%><% else %><% if use_cookies=1 then %><%=Request.Cookies("inno")("email")%><% end if %><% end if %><% end if %>"></td>
</tr>
<tr>
	<td colspan="4" height="1" bgcolor="#cccccc"></td>
</tr>
<tr bgcolor="F7F7F7" height="25"> 
	<td width="110" class="form_title" align="right">HomePage&nbsp;</td>
	<td width="460" colspan="3"><input type="text" name="url" size="30" class="form_input" maxlength="240" value="http://<% if session_login_name = "" and mode="edit" then %><%=url%><% else %><% if session_login_name<>"" then %><%=session_url%><% else %><% if use_cookies=1 then %><%=Request.Cookies("inno")("url")%><% end if %><% end if %><% end if %>"></td>
</tr>
<tr>
	<td colspan="4" height="1" bgcolor="#cccccc"></td>
</tr>
...
<tr bgcolor="F7F7F7" height="25"> 
	<td width="110" class="form_title" align="right">Add function &nbsp;</td>
	<td width="460" colspan="3" class="form_title"><%if nw_level >= session_level then%><input type="checkbox" name="notice" value="1"<% if notice = 1 then %> checked<% end if%>>Announcement<% end if %><input type="checkbox" name="tag" value="<% if content_type <> "1" then %><% if tag = 1 then %>1<% elseif tag = 2 then %>2<% else %>1<% end if %><% else %>1<% end if %>"<% if tag > 0 or (mode<>"edit" and content_type <> 0) then %> checked<% end if%> onClick="check_use_html(this)">Use HTML<input type="checkbox" name="secret" value="<% if secret = 1 then %>1<% elseif secret = 2 then %>2<% else %>1<% end if %>"<% if secret > 0 then %> checked<% end if%> onClick="secret_option(this)">Secret<input type="checkbox" name="reply_mail" value="1"<% if reply_mail = 1 then %> checked<% end if%>>Rcv.Msg.via email<input type="checkbox" name="link" value="1"<% if link <> "" then %> checked<% end if%> onClick="javascript:box()">Link</td>
</tr>
<tr>
	<td colspan="4" height="1" bgcolor="#cccccc"></td>
</tr>
<tr bgcolor="F7F7F7" height="25" id="secret_option" style="display:<% if secret <> 1 then %>none<% end if%>"> 
	<td width="110" class="form_title" align="right">Password for reading &nbsp;</td>
...
<tr bgcolor="F7F7F7" height="25" id="link1" style="display:<% if link = "" then %>none<% end if%>"> 
	<td width="110" class="form_title" align="right">Link #1 &nbsp;</td>
...
<tr bgcolor="F7F7F7" height="25" id="link2" style="display:<% if link = "" then %>none<% end if%>"> 
	<td width="110" class="form_title" align="right">Link #2 &nbsp;</td>
...
<tr bgcolor="F7F7F7" height="25"> 
	<td width="110" class="form_title" align="right"><b><% if board_type = 3 then %>Wether<% else %>Title<% end if %> &nbsp;</b></td>
...
%>
<tr bgcolor="#F7F7F7" height="25">
	<td width="110" class="form_title" align="right">Attached #1 &nbsp;</td>
	<td width="460" colspan="3" class="font1" valign="center"><input type=file name="allfile1" size="30" class="form_input" onkeydown="return false;"> &nbsp; &nbsp; <%=maxsize%><%=b_name%>Byte limit</td>
</tr>
...
<tr bgcolor="#F7F7F7" height="25">
	<td width="110" class="form_title" align="right">Attached #2 &nbsp;</td>
	<td width="460" colspan="3" class="font1" valign="center"><input type=file name="allfile2" size="30" class="form_input" onkeydown="return false;"> &nbsp; &nbsp; <%=maxsize%><%=b_name%>Byte Limit</td>
</tr>
...
<tr bgcolor="#F7F7F7" height="25">
	<td width="110" class="form_title" align="right">Attached #3 &nbsp;</td>
	<td width="460" colspan="3" class="font1" valign="center"><input type=file name="allfile3" size="30" class="form_input" onkeydown="return false;"> &nbsp; &nbsp; <%=maxsize%><%=b_name%>Byte limit</td>
</tr>
...
<tr bgcolor="#F7F7F7" height="25">
	<td width="110" class="form_title" align="right">Attached #4 &nbsp;</td>
	<td width="460" colspan="3" class="font1" valign="center"><input type=file name="allfile4" size="30" class="form_input" onkeydown="return false;"> &nbsp; &nbsp; <%=maxsize%><%=b_name%>Byte Limit</td>
</tr>
...
<table width="<%=board_size%>" border="0" cellpadding="0" cellspacing="0">
<tr>
	<td align="right" style="word-break:break-all;padding:5px;"><a href="javascript:submit();"><img src="../img/but_<% if mode = "edit" then %>edit<% else %>save<% end if %>.gif" border="0"></a> <a href="javascript:reset();"><img src="../img/but_again.gif" border="0"></a><a href="javascript:history.go(-<% if mode = "edit" and session_login_name = "" then %>2<% else %>1<% end if %>);"><img src="../img/but_cancel.gif" border="0"></a><br></td>
</tr>
</table>
