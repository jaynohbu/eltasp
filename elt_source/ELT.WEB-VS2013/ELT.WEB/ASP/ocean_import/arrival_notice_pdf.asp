...
Dim vDepPort,vArrPort,vSubHAWB
...
'if Not vHAWB="" then
 if true then 
	...
	if  vInvoiceNo="" then  
        SQL="select a.sub_mawb1 as sub_master, a.it_number as itno,a.it_date as itdate,a.it_entry_port as itentryport,a.cargo_location as cl,a.sec as sec,a.*,a.process_dt as process_dt_h,a.mawb_num as mawb_num,b.*,isnull(a.dep_port,'') as dep_port_h,isnull(a.arr_port,'') as arr_port_h,a.eta as eta_h,a.etd as etd_h from import_hawb a,import_mawb b where a.elt_account_number=b.elt_account_number and a.elt_account_number=" & elt_account_number & " and a.mawb_num='"&vMAWB&"'"&" and a.iType=b.iType and a.iType='" & iType & "' and a.mawb_num=b.mawb_num and a.hawb_num='" & vHAWB & "' and a.sec=b.sec order by sec desc"
	else 
		SQL="select a.sub_mawb1 as sub_master,  a.it_number as itno,a.it_date as itdate,a.it_entry_port as itentryport,a.cargo_location as cl,a.sec as sec,a.*,a.process_dt as process_dt_h,a.mawb_num as mawb_num,b.*,isnull(a.dep_port,'') as dep_port_h,isnull(a.arr_port,'') as arr_port_h,a.eta as eta_h,a.etd as etd_h from import_hawb a,import_mawb b where a.elt_account_number=b.elt_account_number and a.elt_account_number=" & elt_account_number & " and a.mawb_num='"&vMAWB&"'"&" and a.iType=b.iType and a.iType='" & iType & "' and a.mawb_num=b.mawb_num and a.hawb_num='" & vHAWB & "' and a.sec=b.sec and a.invoice_no="&vInvoiceNo
	end if 
	rs.Open SQL, eltConn, , , adCmdText
'	response.Write "---------"& SQL
'	response.End
	...
	if Not rs.EOF then
		...
		if IsNull(InvoiceNo) Then
			...
			InvoiceNo=FormatNumberPlus(checkblank(InvoiceNo,"0"),0)
		end if
...
'////////////////////////////////// 
	...
	if Not InvoiceNo=0 then
		...
		Do While Not rs.EOF
			...
			vTotalAmount=vTotalAmount+FormatNumberPlus(checkblank(aAmount(tIndex),0),2)
			rs.MoveNext
			tIndex=tIndex+1
		Loop
		rs.Close
...
PDF.SetFormFieldData "SubHAWB",vSubMAWB,0
...
If Not doInvoice = "no" then 
    for i=0 to NoItem-1
        If FormatNumberPlus(checkblank(aAmount(i),"0"),0)<> 0 then 
