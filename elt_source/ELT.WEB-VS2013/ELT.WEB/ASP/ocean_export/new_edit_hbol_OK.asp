...
<br>
      <form method="post" name="frmHAWB">
		<input type="hidden" name="hHBOLPrefix" value="<%= vHBOLPrefix %>">
	 	<input type="hidden" name="hNewHAWB" value="<%= vHBOL %>">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" class="bodycopy">
                ...
                <tr>
                  <td height="22" align="center">&nbsp;<font color="c16b42"><strong>House B/L No</strong></font>.&nbsp;&nbsp;
                    <select name="lstHBOLPrefix" size="1" class="smallselect lstHBOLPrefix" style="WIDTH: 60px" onChange="PrefixChange()">
                      <% For i=0 To pIndex-1 %>
...
function OkClick(){ // converted from vbscript
    ...
    var errHAWB = "<%= errHAWB %>";
    var vHBOL = document.frmHAWB.txtHAWB.value;

    var tmpHBOL
    tmpHBOL = vHBOL;
    var pos=vHBOL.indexOf("-");
    if (pos>0 )
        tmpHBOL=vHBOL.substring(pos+1,21);


    if (tmpHBOL!="" && !IsNumeric(tmpHBOL) ){
    alert( "Please enter HBOL# in Prefix-Number format!");
    return;
    }
    ...
    var sIndex=document.frmHAWB.lstHBOLPrefix.selectedIndex;
    var HAWBPrefix= $("select.lstHBOLPrefix>option").get(sIndex).text;
    var NEXTPrefix= $("select.lstHBOLPrefix>option").get(sIndex).value;

    document.frmHAWB.action="new_edit_hbol_ok.asp?save=yes&prefix=" 
            + encodeURIComponent(HAWBPrefix) +"&HBOL=" + encodeURIComponent(vHBOL)
...
function PrefixChange() // converted from vbscript
{
    ...
    var NEXTPrefix= $("select.lstHBOLPrefix>option").get(sIndex).value;
    var vHBOL = "";
	document.frmHAWB.action="new_edit_hbol_ok.asp?SaveAsNew=yes&prefix=" 
        + encodeURIComponent(HAWBPrefix) + "&HBOL=" 
        + encodeURIComponent(vHBOL) + "&NEXTPREFIX=" 
        + encodeURIComponent(NEXTPrefix);
	document.frmHAWB.method="POST";
	document.frmHAWB.target="_self";
	frmHAWB.submit();
}
