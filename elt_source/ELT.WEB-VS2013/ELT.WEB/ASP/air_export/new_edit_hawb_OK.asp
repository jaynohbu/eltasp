...
DIM AddWC,AddOC,Edit,NewHAWB,Save,SaveAsNew,IsCopied,DeleteWC,DeleteOC,vHAWBPrefix,rURL,AutoSave,DeleteHAWB,qCancel
...
<script type="text/jscript">
    parent.document.fShowModal.hReturnValue.value = '<%= vHAWB %>';
    parent.myUnLoad();
...
SUB DELETE_TEMP_HAWB( saved_HAWB_NUM )
	SQL= "delete hawb_master where elt_account_number = " & elt_account_number _
	    & " and hawb_num=N'" & saved_HAWB_NUM & "'" & " and is_dome='N' and CAST(Agent_Info AS VARCHAR)='Auto Generated No.'"	
	eltConn.Execute SQL	
END SUB
%>
<%
...
'get Hawb prefix from user profile
	SQL= "select prefix,next_no from user_prefix where elt_account_number=" _
	    & elt_account_number & " and type='HAWB' order by seq_num"
...
<body link="336699" vlink="336699" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"
    ...
    <form method="post" name="frmHAWB">
        ...
        <input type="hidden" name="hHAWB"  id="hHAWB"  value="<%= vHAWB %>">
        <input type="hidden" name="herrHAWB" id="herrHAWB" value="<%= errHAWB %>">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF"
            class="bodycopy">
            <tr>
                <td class="formbody">
                    <div align="center">
                        * Next bill number :</div>
                </td>
            </tr>
            <tr>
                <td class="formbody">
            ...
            <tr>
                <td height="22" align="center">
                    &nbsp;<font color="c16b42"><strong>Bill No.</strong></font>&nbsp;&nbsp;
                    <select name="lstHAWBPrefix" size="1" class="smallselect lstHAWBPrefix" style="width: 60px" onChange="PrefixChange()">
                        <% For i=0 To pIndex-1 %>
                    ...
                    <%if HAWB_Exists then%>
                    <input name="txtHAWB" id="txtHAWB" class="shorttextfieldbold" style="width: 100px; color: #FF0000"
                        tabindex="1" value="<%= vHAWB %>" size="20">
                    <%else%>
                    <input name="txtHAWB" id="txtHAWB" class="shorttextfieldbold" style="width: 100px" tabindex="1"
                        value="<%= vHAWB %>" size="20">
                    <%end if%>
                </td>
            </tr>
            ...
            <tr>
                <td height="18" align="center">
                    <P align="center">
                        <span class="style3">Please choose another number! </span>
                    </P>
                </td>
            </tr>
            <%end if%>
        ...
        <% If pIndex > 0 Then %>
        <P align="center">
            <input name="OK" id="OK" type="button" id="OK" value="OK" onClick="OkClick()" style="width: 70px">
            <input name="Cancel" id="Cancel"  type="button" id="Cancel" value="Cancel" onClick="CancelClick()"
                style="width: 70px">
        </P>
        <% Else %>
        <P align="center">
            <span class="style3">Please, enter a prefix in company information page</span></P>
        <% End If %>
    </form>
</body>
...
function OkClick () // converted from vbscript
{
    <% If IsCopied = "yes" Then %>
    ...
    errHAWB = "<%= errHAWB %>";
    vHAWB = document.frmHAWB.txtHAWB.value;
    ...
    if (pos>0 ){
	    tmpHAWB=vHAWB.substring(pos+1,21);
	    if (tmpHAWB.length >9 ){
	        alert( "The To HAWB NO has less then 10 characters!");
	        return;
	    }
    }
    ...
    HAWBPrefix= $("select.lstHAWBPrefix>option").get(sindex).text;
    NEXTPrefix= $("select.lstHAWBPrefix>option").get(sindex).value;
    document.frmHAWB.action="new_edit_hawb_OK.asp?save=yes&prefix=" +encodeURIComponent(HAWBPrefix) + "&HAWB=" + encodeURIComponent(vHAWB) + "&NEXTPREFIX=" + encodeURIComponent(NEXTPrefix) ;
    document.frmHAWB.method="POST";
    document.frmHAWB.target="_self";
    document.frmHAWB.submit();
}



function CancelClick(){ // converted from vbscript
...
function PrefixChange(){ // converted from vbscript
    ...
    NEXTPrefix= $("select.lstHAWBPrefix>option").get(sindex).value;
    vHAWB = "";
    document.frmHAWB.action="new_edit_hawb_ok.asp?SaveAsNew=yes&prefix=" 
        + encodeURIComponent(HAWBPrefix) 
        + "&HAWB="+ encodeURIComponent(vHAWB) 
        + "&NEXTPREFIX=" + encodeURIComponent(NEXTPrefix) ;
    document.frmHAWB.method="POST";
    document.frmHAWB.target="_self";
    frmHAWB.submit();
}
