...
if Not Branch=0 then
	if vCompany=0 then
		SQL= "select * from bill where elt_account_number = " & Branch & " and bill_date<" & DM & ToDate+1 & DM & " and bill_amt_due>0  order by vendor_name,bill_date desc"
	else
		SQL= "select * from bill where elt_account_number = " & Branch & " and bill_date<" & DM & ToDate+1 & DM & " and bill_amt_due>0 and vendor_number=" & vCompany & " order by bill_date desc"
	end if
else
	SQL= "select * from bill where Left(elt_account_number,5) = " & Left(elt_account_number,5) & " and bill_date<" & DM & ToDate+1 & DM & " and bill_amt_due>0  order by vendor_name,bill_date desc"
end if
rs.Open SQL, eltConn, adOpenStatic, , adCmdText
tIndex=0
'rs.MoveLast
...
Do While Not rs.EOF And tIndex<Count
	...
	'vTerm=rs("term_curr")
	vInvoiceDate=rs("bill_due_date")
	vBalance=cDbl(rs("bill_amt_due"))
...
<table width="95%" border="0" align="center" cellpadding="2" cellspacing="0">
  <tr> 
    <td height="20" align="left" valign="bottom" class="pageheader">a/p Aging 
      Summary </td>
  </tr>
</table>
...
Sub PrintReport()
	jPopUpNormal()
	document.form1.action="ap_aging_summPrint.asp" & "?WindowName=" & window.name
