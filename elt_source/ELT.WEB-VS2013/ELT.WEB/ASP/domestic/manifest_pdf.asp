...
'///move here by iMoon
r = PDF.AddLogo(Server.MapPath("../template") &  "/logo/Logo" & elt_account_number & "_horizon.pdf",1)
...
<%
    ...
    If rs.EOF Then
        rs.close
        If vAgent=0 Or vAgent=MasterAgentNo Then
            SQL= "select NULL AS sub_to_no,'N' AS is_sub,mawb_num,'' as hawb_num,'' as agent_name,Total_Pieces,adjusted_Weight,Weight_Scale,Shipper_Info,Shipper_name,Consignee_Info,Consignee_Name,Desc2 AS manifest_desc,'' as aes_xtn,account_info,'' as danger_good,'' as sed_stmt from mawb_master where elt_account_number = " & elt_account_number & " and mawb_num = N'" & vMAWB & "' order by hawb_num"
        Else
