...
<td width="762" align="left" valign="middle" class="bodyheader"><input class="bodycopy" id="bUclear" style="width: 80px; height:22px" type="button" value="Incomplete" onClick="javascript:doBtn(this);"></td>				  
			    ...
			    <tr>
			        <td></td>
			        <td colspan="9">
                    <% DIM selChk ,p,CurrentPage,CurrentPageRows,j
                      Dim RowsPerPage,TotalRows,PageCount,LastPageCount
                       RowsPerPage = 50
                       TotalRows= code_list.count
                        PageCount = 1 
                        LastPageCount = TotalRows Mod RowsPerPage
                        If TotalRows > RowsPerPage Then
                            'TotalCount = 170
                            PageCount = (TotalRows-LastPageCount)/RowsPerPage   ' (3*50)+20
                ...
                <tr>
                    ...
                    <span style="padding:4px;cursor:pointer" onclick="javascript:DisplayResultTable(<%=p %>);" ><%=p %></span>
                    <% Next %>
                    </td>
                </tr>
              </table>
                  ...
                  For j= 1 To PageCount %>
                    ...
                    width="100%" border="0" cellpadding="1" cellspacing="1">
                      ...
                      If CurrentPage < PageCount Then
                        CurrentPageRows = RowsPerPage
                        Else
                        CurrentPageRows = LastPageCount
                        End If
                      %>
                     <tr>
                        <td colspan="12" style="text-align:right">
                             <%=CurrentPage %> /   <%=PageCount %> Page(s)
                          (Total <%=TotalReturnCount %> Records)
                          (<%=(((CurrentPage-1)*RowsPerPage)) %>
                          ~<%= (((CurrentPage-1)*RowsPerPage)-1+CurrentPageRows) %>)
                        </td>
                     </tr>
                      ...
                      <% for i= (((CurrentPage-1)*RowsPerPage)) To (((CurrentPage-1)*RowsPerPage)-1+CurrentPageRows) %>
					  ...
					  %>
						  ...
						  <% end if %>
                        <td style="width:10px;cursor:hand" onClick="DoChecking('<%=code_list(i)("tran_seq_num")%>');"><img border="0" align="absmiddle" src="../images/checkbox_<% if selChk then  response.write("o") else response.write("n") end if%>.gif" id='img<%=code_list(i)("tran_seq_num")%>' checkeditem="<% if selChk then  response.write("Y") else response.write("N") end if%>" onClick="ItemWasChecked(this);" WIDTH="10" HEIGHT="10"><input type="hidden" name="chk" id='chkimg<%=code_list(i)("tran_seq_num")%>' seq='<%=code_list(i)("tran_seq_num")%>' class='<%=code_list(i)("tran_seq_num")%>' <% if selChk then response.write (" value='off'") %>></td>
                ...
                <% next %>
                  ...
                  <% else %>
	<span style="font-size: 10px;font-weight: bold;color: #000000" class="bodycopy">No data was found.</span><br><br><a href="javascript:" onclick="javascript:self.location='/ASP/acct_tasks/manage_chk.asp?PostBack=false';return false;">Please click here to return.</a>
                  <% end if %>
