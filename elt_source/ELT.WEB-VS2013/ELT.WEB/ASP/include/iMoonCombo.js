...
// Add New 10/30/2006
        ...
        if(button != null){
            ...
            if(temp !="")
            {
                ...
                initialLeftPosition = parseInt(temp);       
            }
             div.style.left = initialLeftPosition+ tWidth + 140 + 'px';
    ...
    function ComboBox_EnsureListSize( theList ) {
        if ( typeof( theList.listSize ) == "undefined" ) {
            ...
            if( theList.children.length > 0 ) {
                theList.listSize = theList.children.length;
                return;
            }
            theList.listSize = 4;
        }
...
// Add New 10/30/2006
    ...
    function ComboBox_ShowList(e) {
        if ( !this.IsShowing && !this.disabled ) {
			...
			oScroll(this,this.options.length +1);
