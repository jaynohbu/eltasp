...
Sub Register_new_edit_hawb( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/air_export/new_edit_hawb.asp?Edit=YES&HAWB=" & vHAWB
...
Sub Register_new_edit_mawb( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/air_export/new_edit_mawb.asp?Edit=YES&MAWB=" & vMAWB
...
Sub Register_new_edit_hbol( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/ocean_export/new_edit_hbol.asp?Edit=YES&HBOL=" & vHBOL
...
Sub Register_new_edit_mbol( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/ocean_export/new_edit_mbol.asp?Edit=YES&BookingNum=" & vBookingNum
...
Sub Register_new_edit_invoice( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/acct_tasks/edit_invoice.asp?edit=yes&InvoiceNo=" & InvoiceNo
...
Sub Register_air_import2( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/air_import/air_import2.asp?iType=" & iType & "&Edit=yes&MAWB=" & vMAWB & "&Sec=" & vSec & "&AgentOrgAcct=" & vAgentOrgAcct
...
Sub Register_ocean_import2( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/ocean_import/ocean_import2.asp?iType=" & iType & "&Edit=yes&MAWB=" & vMAWB & "&Sec=" & vSec & "&AgentOrgAcct=" & vAgentOrgAcct
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
...
Sub Register_new_edit_air_book( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/air_export/booking.asp?Edit=yes&MAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
...
Sub Register_new_edit_ocean_book( strSoTitle )
	if	Save="yes" then
		sURL = 	"/ASP/ocean_export/booking.asp?Edit=yes&lstBookingNum=" & vBookingNum
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
...
Sub Register_new_air_pre_alert( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/air_export/pre_alert.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_new_ocean_pre_alert( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/ocean_export/pre_alert.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_new_air_shipping_notice( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/air_export/shipping_notice.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_new_ocean_shipping_notice( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/ocean_export/shipping_notice.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_air_import_proof_delivery( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/air_import/proof_delivery.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_ocean_import_proof_delivery( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/ocean_import/proof_delivery.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_ocean_import_earrival_notice( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/ocean_import/earrival_notice.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_air_import_earrival_notice( strSoTitle )
	if	Send="yes" then
		sURL = 	"/ASP/air_import/earrival_notice.asp?rEdit=yes&rMAWB=" & vMAWB
		call recentRegisterDB (sURL, vMAWB,"", strSoTitle)
	end if
End Sub
...
Sub Register_co_config( strSoTitle )
	if	Save="yes" then
		sURL = 	"/SiteAdmin/CompanyInformation"
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/master_data/edit_port.asp"
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/master_data/edit_freight.asp"
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/master_data/edit_sche_b.asp"
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/master_data/new_edit_mawb.asp"
	if	Go="yes" Then
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/acct_tasks/receiv_pay.asp?PaymentNo="&recentPaymentNo
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/acct_tasks/enter_bill.asp?ViewBill=yes&BillNo=" & BillNo
...
Dim strDesc,strCode
	...
	sURL = 	"/ASP/acct_tasks/pay_bills.asp?EditCheck=yes&CheckQueueID="&PrintID
...
'	sURL = 	"/ASP/acct_tasks/write_chk.asp?EditCheck=yes&CheckQueueID=" & PrintID & "&VendorNo=" & VendorNo & "&Bank=" & Bank 
	sURL = 	"/ASP/acct_tasks/write_chk.asp" 
	if	Save="yes" Then
