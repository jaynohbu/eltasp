...
var	Wedit1 = document.getElementById('txtStartDate')
var	Wedit2 = document.getElementById('txtEndDate')
...
date = now.getDate(); 
				...
				if(r== 'Select' ) { 
					...
					Wedit1.value=convertDateFormat(now);				 
				    //var converted;
                    // var iDay = now.getDate();
	                // var iMonth = now.getMonth()+1;
                    //var iYear = now.getFullYear();
	                converted = DateAdd("d",1,Wedit1.value);
	                converted=convertDateFormat(converted);
					Wedit2.value=converted;
				}

				else if(r== 'Month to Date') {				

					fromDate = new Date(year,month,1);
					Wedit1.value=convertDateFormat(fromDate);
					Wedit2.value=convertDateFormat(now);
				}
					...
					Wedit2.value=convertDateFormat(now);
						...
						fromDate=new Date(year,month,1);
						toDate = new Date(year,month+1,0);// 1: starting date of the year,0: the last day of the last month, -1 and so on. according to the day the month changes
						Wedit1.value=convertDateFormat(fromDate);
						Wedit2.value=convertDateFormat(toDate);
				}
