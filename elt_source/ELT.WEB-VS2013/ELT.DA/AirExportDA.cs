...
namespace ELT.DA
{ 
    public class AirExportDA:DABase
    {  
     ...
     public List<AirExportDocument> GetAirExportDocumentList(int ELT_ACCOUNT_NUMBER, string BillNum, string BillType)
        {
            SqlConnection conn = new SqlConnection(GetConnectionString(AppConstants.DB_CONN_PROD));
            SqlCommand cmd = new SqlCommand() { Connection = conn, CommandType = CommandType.StoredProcedure, CommandText = "dbo.[GetAirExportDocumentList]" };
            List<AirExportDocument> list = new List<AirExportDocument>();
            ...
            try
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                            ...
                            AirExportDocument item = new AirExportDocument() {
