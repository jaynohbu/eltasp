...
namespace ELT.BL
{
    public class DomesticFreightBL
    {
        DomesticFreightDA DomesticFreightDA = new DomesticFreightDA();
        public List<ManifestAgentInfo> GetAgentsInDmMAWB(int ELT_ACCOUNT_NUMBER, string mawb)
        {
            return DomesticFreightDA.GetAgentsInDmMAWB(ELT_ACCOUNT_NUMBER, mawb);
        }

        public List<EmailItem> GetDomesticFreightAgentPreAlertEmailItems(int ELT_ACCOUNT_NUMBER, string BillNum, BillType billType)
        {
            List<EmailItem> eitems = new List<EmailItem>();
            var hbItems = DomesticFreightDA.GetDomesticFreightDocumentList(ELT_ACCOUNT_NUMBER, BillNum, billType.ToString().Split('_')[1]);
            if (hbItems.Count() > 0)
            {
                ...
                foreach (var agent in agents)
                {
                    ...
                    EmailItem eitem = new EmailItem() { RecipientRelation = "Agent", Attachments = new List<EmailAttachment>(), RecipientEmail = hawbs[0].agent_email, RecipientName = hawbs[0].agent_name, EmailType = EmailType.DomesticFreight_Agent_PreAlert, IsSelected = true, EmailItemID = itemid++ };

                    eitem.Attachments.Add(new EmailAttachment() { IsSelected = true, GeneratorPath = string.Format("~/ASP/domestic/mawb_pdf.asp?MAWB={0}&Copy=CONSIGNEE", hbItems[0].mawb_num), FileName = string.Format("MAWB {0}.pdf", hbItems[0].mawb_num) });

                    if (hbItems[0].isDirect != "Y")
                    {
                        eitem.Attachments.Add(new EmailAttachment() { IsSelected = true, GeneratorPath = string.Format("~/ASP/domestic/manifest_pdf.asp?MAWB={0}&Agent={1}&MasterAgentNo={2}", hbItems[0].mawb_num, agent, hbItems[0].master_agent), FileName = string.Format("Manifest {0}.pdf", hbItems[0].mawb_num) });

                        foreach (var h in hawbs)
                        {
                            eitem.Attachments.Add(new EmailAttachment() { IsSelected = true, GeneratorPath = string.Format("~/ASP/domestic/hawb_pdf.asp?HAWB={0}&Copy=CONSIGNEE", h.hawb_num), FileName = string.Format("HAWB {0}.pdf", h.hawb_num) });
        ...
        public List<EmailItem> GetDomesticFreightShippingNoticeEmailItems(int ELT_ACCOUNT_NUMBER, string BillNum, BillType billType)
        {
            List<EmailItem> eitems = new List<EmailItem>();
            var hbItems = DomesticFreightDA.GetDomesticFreightDocumentList(ELT_ACCOUNT_NUMBER, BillNum, billType.ToString().Split('_')[1]);
            if (hbItems.Count() > 0)
            {
                ...
                foreach (var shipper in shippers)
                {
                    ...
                    EmailItem eitem = new EmailItem() { RecipientRelation = "Shipper", Attachments = new List<EmailAttachment>(), RecipientEmail = hawbs[0].shipper_email, RecipientName = hawbs[0].shipper_name, EmailType = EmailType.DomesticFreight_Agent_PreAlert, IsSelected = true, EmailItemID = itemid++ };
                    if (hbItems[0].isDirect != "Y")
                    {
                        foreach (var h in hawbs)
                        {
